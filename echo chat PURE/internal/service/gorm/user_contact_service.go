...
// 本文件实现 user contact service 相关的业务服务，负责组织数据库、缓存和业务规则。
...
func (u *userContactService) GetContactInfo(contactId string) (string, respond.GetContactInfoRespond, int) {
	if contactId[0] == 'G' {
		...
		zlog.Debug(
			"contact.info.loaded_user",
			zap.String("event", "contact.info.loaded_user"),
			zap.String("module", "contact.service"),
			zap.String("contact_id", user.Uuid),
