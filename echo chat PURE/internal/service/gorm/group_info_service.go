...
// 本文件实现 group info service 相关的业务服务，负责组织数据库、缓存和业务规则。
...
func (g *groupInfoService) UpdateGroupInfo(req request.UpdateGroupInfoRequest) (string, int) {
	...
	for _, session := range sessionList {
		...
		zlog.Debug(
			"group.session.sync",
			zap.String("event", "group.session.sync"),
			zap.String("module", "group.service"),
			zap.String("session_uuid", session.Uuid),
			zap.String("group_id", req.Uuid),
		)
...
func (g *groupInfoService) RemoveGroupMembers(req request.RemoveGroupMembersRequest) (string, int) {
	...
	deletedAt.Valid = true
	zlog.Info(
		"group.member.remove.request",
		zap.String("event", "group.member.remove.request"),
		zap.String("module", "group.service"),
		zap.String("group_id", req.GroupId),
		zap.String("owner_id", req.OwnerId),
		zap.Int("remove_count", len(req.UuidList)),
	)
