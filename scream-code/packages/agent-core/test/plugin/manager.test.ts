...
async function makeScreamHome(): Promise<string> {
  return mkdtemp(path.join(tmpdir(), 'scream-home-'));
}
...
describe('PluginManager', () => {
  it('install() adds a plugin and load() rehydrates it from disk', async () => {
    ...
    let manager = new PluginManager({ screamHomeDir: home });
    await manager.load();
    ...
    manager = new PluginManager({ screamHomeDir: home });
    await manager.load();
    expect(manager.list()).toHaveLength(1);
  ...
  it('install() accepts a .scream-plugin manifest', async () => {
    const home = await makeScreamHome();
    const root = await mkdtemp(path.join(tmpdir(), 'scream-plugin-'));
    ...
    expect(record.manifestKind).toBe('scream-plugin-dir');
  ...
  it('install() copies a symlinked plugin root into the managed plugins dir', async () => {
    ...
    const reloaded = new PluginManager({ screamHomeDir: home });
    await reloaded.load();
  ...
  it('reload() picks up edits to the managed plugin copy', async () => {
    ...
    await writeFile(
      path.join(managedRoot, 'scream.plugin.json'),
  ...
  it('keeps a plugin in error state instead of losing it on a broken manifest', async () => {
    ...
    await manager.install(root);
    await writeFile(
      path.join(await managedPluginRoot(home, 'demo'), 'scream.plugin.json'),
      '{ not json',
      'utf8',
    );
    ...
    expect(manager.enabledMcpServers()).toEqual(
      expect.objectContaining({
        'plugin-demo:finance': expect.objectContaining({
          command: 'finance-mcp',
          cwd: managedRoot,
          env: expect.objectContaining({ SCREAM_CODE_HOME: home, SCREAM_PLUGIN_ROOT: managedRoot }),
        }),
  ...
  it('install() supports zip URL', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'plugin/scream.plugin.json',
  ...
  it('install() from zip-url overwrites existing zip-url plugin', async () => {
    ...
    const zipBuffer1 = await createZipBuffer([
      { name: 'plugin/scream.plugin.json', data: JSON.stringify({ name: 'zip-demo', version: '1.0.0' }) },
    ]);
    ...
    const zipBuffer2 = await createZipBuffer([
      { name: 'plugin/scream.plugin.json', data: JSON.stringify({ name: 'zip-demo', version: '2.0.0' }) },
    ]);
  ...
  it('install() from github URL resolves latest release and records github metadata', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'wbxl2000-superpowers-abc/scream.plugin.json',
  ...
  it('install() from /tree/<tag-shaped-ref> downloads via short form, not refs/heads/ (P1 regression)', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'obra-superpowers-v5.1.0/scream.plugin.json',
  ...
  it('install() from github /tree/<branch> bypasses the GitHub API', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'wbxl2000-superpowers-main/scream.plugin.json',
  ...
  it('install() from github URL overwrites an existing zip-url install (CDN migration)', async () => {
    ...
    const cdnZip = await createZipBuffer([
      { name: 'pkg/scream.plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.0.0' }) },
    ]);
    ...
    const ghZip = await createZipBuffer([
      { name: 'pkg/scream.plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.1.0' }) },
    ]);
    using _ = mockGithubFetch({
      releaseTag: 'v5.1.0',
      tarball: ghZip,
