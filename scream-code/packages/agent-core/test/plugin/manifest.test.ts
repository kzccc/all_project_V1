...
async function makePlugin(
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'scream-plugin-test-'));
...
describe('parseManifest', () => {
  it('reads a minimal scream.plugin.json at the plugin root', async () => {
    const root = await makePlugin({
      'scream.plugin.json': JSON.stringify({ name: 'demo', version: '1.0.0' }),
    });
    const result = await parseManifest(root);
    expect(result.manifest?.name).toBe('demo');
    expect(result.manifest?.version).toBe('1.0.0');
    expect(result.manifestKind).toBe('scream-plugin-root');
  ...
  it('prefers root scream.plugin.json when .scream-plugin/plugin.json also exists', async () => {
    const root = await makePlugin({
      ...
      '.scream-plugin/plugin.json': JSON.stringify({ name: 'dir-version' }),
    });
    const result = await parseManifest(root);
    expect(result.manifestKind).toBe('scream-plugin-root');
    expect(result.manifest?.name).toBe('root-version');
    expect(result.shadowedManifestPath).toBe(path.join(root, '.scream-plugin/plugin.json'));
  });

  it('falls back to .scream-plugin/plugin.json when scream.plugin.json is absent', async () => {
    const root = await makePlugin(
      {
        '.scream-plugin/plugin.json': JSON.stringify({
          name: 'demo',
          version: '1.0.0',
          keywords: ['workflow'],
          skills: './skills/',
    ...
    const result = await parseManifest(root);
    expect(result.manifestKind).toBe('scream-plugin-dir');
    expect(result.manifestPath).toBe(path.join(root, '.scream-plugin/plugin.json'));
    expect(result.manifest?.name).toBe('demo');
    expect(result.manifest?.version).toBe('1.0.0');
  ...
  it('does NOT fall back to .scream-plugin/plugin.json when scream.plugin.json is invalid JSON', async () => {
    const root = await makePlugin({
      'scream.plugin.json': '{ not json',
      '.scream-plugin/plugin.json': JSON.stringify({ name: 'dir-version' }),
    });
    const result = await parseManifest(root);
  ...
  it('rejects names that violate the regex', async () => {
    const root = await makePlugin({
      'scream.plugin.json': JSON.stringify({ name: 'Bad Name!' }),
    });
    const result = await parseManifest(root);
    expect(result.manifest).toBeUndefined();
    expect(result.diagnostics).toContainEqual(
  ...
  it('resolves a single skills path', async () => {
    const root = await makePlugin(
      { 'scream.plugin.json': JSON.stringify({ name: 'demo', skills: './skills/' }) },
      { dirs: ['skills'] },
    );
    const result = await parseManifest(root);
    expect(result.manifest?.skills).toEqual([path.join(root, 'skills')]);
  ...
  it('rejects a skills path that escapes plugin_root', async () => {
    const root = await makePlugin({
      'scream.plugin.json': JSON.stringify({ name: 'demo', skills: './../escape' }),
    });
    const result = await parseManifest(root);
    expect(result.diagnostics).toContainEqual(
      expect.objectContaining({
  ...
  it('rejects a skills path that escapes via a symlink', async () => {
    const root = await makePlugin({
      'scream.plugin.json': JSON.stringify({ name: 'demo', skills: './sym' }),
    });
    const outside = await mkdtemp(path.join(tmpdir(), 'scream-plugin-outside-'));
  ...
  it('warns when skills resolves to a non-directory', async () => {
    const root = await makePlugin({
      'scream.plugin.json': JSON.stringify({ name: 'demo', skills: './notes.md' }),
      'notes.md': 'hi',
    });
    const result = await parseManifest(root);
    expect(result.diagnostics).toContainEqual(
  ...
  it('falls back to root SKILL.md when skills field is absent', async () => {
    const root = await makePlugin({
      'scream.plugin.json': JSON.stringify({ name: 'demo' }),
      'SKILL.md': '---\nname: root-skill\n---\nbody',
    });
    const result = await parseManifest(root);
  ...
  it('parses skillInstructions', async () => {
    const root = await makePlugin({
      'scream.plugin.json': JSON.stringify({ name: 'demo', skillInstructions: 'Do this.' }),
    });
    const result = await parseManifest(root);
  ...
  it('parses keywords metadata', async () => {
    const root = await makePlugin({
      'scream.plugin.json': JSON.stringify({ name: 'demo', keywords: ['finance', 'workflow'] }),
    });
    const result = await parseManifest(root);
