...
function fakeProcess(stdout: string, exitCode = 0): JianProcess {
  return {
...
function gitJian(script: GitScript): Jian {
  return createFakeJian({
    exec: async (...args: string[]): Promise<JianProcess> => {
...
describe('collectGitContext', () => {
  it('returns an empty string when the directory is not a git repository', async () => {
    const jian = gitJian({ 'rev-parse': { stdout: '', exitCode: 1 } });
    expect(await collectGitContext(jian, '/project')).toBe('');
  });
  ...
  it('builds a git-context block with all sections', async () => {
    ...
    const block = await collectGitContext(jian, '/project');
  ...
  it('returns an empty string when only the working directory is known', async () => {
    const jian = gitJian({ 'rev-parse': { stdout: 'true' } });
    expect(await collectGitContext(jian, '/project')).toBe('');
  });
  ...
  it('treats a hanging git command as a failure (timeout)', async () => {
    ...
    try {
      const jian = createFakeJian({
        exec: async (): Promise<JianProcess> => {
      ...
      const promise = collectGitContext(jian, '/project');
