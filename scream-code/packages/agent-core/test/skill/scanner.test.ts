...
describe('skill discovery', () => {
  it('resolves documented roots in precedence order with brand merging enabled by default', async () => {
    ...
    await mkdir(join(repoDir, '.scream-code', 'skills'), { recursive: true });
    await mkdir(join(repoDir, '.agents', 'skills'), { recursive: true });
    await mkdir(join(homeDir, '.scream-code', 'skills'), { recursive: true });
    await mkdir(join(homeDir, '.agents', 'skills'), { recursive: true });
    await mkdir(join(repoDir, 'team-skills'), { recursive: true });
    ...
    expect(roots.map((root) => relative(realRepoDir, root.path))).toEqual([
      '.scream-code/skills',
      '.agents/skills',
      relative(realRepoDir, await realpath(join(homeDir, '.scream-code', 'skills'))),
      relative(realRepoDir, await realpath(join(homeDir, '.agents', 'skills'))),
      'team-skills',
    ]);
  ...
  it('uses only the first brand directory when brand merging is disabled', async () => {
    ...
    expect(roots.map((root) => root.path)).toEqual([
      await realpath(join(repoDir, '.scream-code', 'skills')),
      await realpath(join(homeDir, '.scream-code', 'skills')),
    ]);
  });
  ...
  it('lets explicit skill dirs replace automatic project and user discovery', async () => {
    ...
    await mkdir(join(repoDir, '.scream-code', 'skills'), { recursive: true });
    await mkdir(join(homeDir, '.scream-code', 'skills'), { recursive: true });
    await mkdir(join(repoDir, 'explicit-skills'), { recursive: true });
    await mkdir(join(repoDir, 'extra-skills'), { recursive: true });
    ...
    expect(roots.map((root) => [relative(realRepoDir, root.path), root.source])).toEqual([
  ...
  it('discovers flat markdown skills, keeps directory skills over same-name flat files, and preserves source precedence', async () => {
    ...
    const userRoot = join(homeDir, '.scream-code', 'skills');
    await writeSkill(projectRoot, 'review.md', ['Project review body first line.', '', 'Details.']);
    await writeSkill(userRoot, join('review', 'SKILL.md'), [
      '---',
      'name: review',
    ...
    await writeSkill(projectRoot, join('deploy', 'SKILL.md'), [
      '---',
      'name: deploy',
  ...
  it('keeps flow skills user-visible while excluding them from model invocation', async () => {
    ...
    const projectRoot = join(repoDir, '.scream-code', 'skills');
    await writeSkill(projectRoot, join('review-flow', 'SKILL.md'), [
      '---',
      'name: review-flow',
  ...
  it('skips directory skills with missing frontmatter metadata', async () => {
    const { repoDir } = await makeWorkspace();
    const projectRoot = join(repoDir, '.scream-code', 'skills');
    await writeSkill(projectRoot, join('valid', 'SKILL.md'), [
      '---',
      'name: valid',
    ...
    await writeSkill(projectRoot, join('no-frontmatter', 'SKILL.md'), [
      '# Heading should not become a description',
      '',
      'Body.',
    ]);
    await writeSkill(projectRoot, join('missing-name', 'SKILL.md'), [
      '---',
    ...
    await writeSkill(projectRoot, join('missing-description', 'SKILL.md'), [
      '---',
...
describe('discoverSkills shape and ordering', () => {
  it('parses frontmatter name/description for subdir skills', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.scream-code', 'skills');
    await writeSkill(root, join('alpha', 'SKILL.md'), [
      '---',
      'name: alpha-skill',
  ...
  it('keeps the first roots version when a skill name appears in multiple roots', async () => {
    const { repoDir } = await makeWorkspace();
    const rootA = join(repoDir, 'root_a');
    const rootB = join(repoDir, 'root_b');
    await writeSkill(rootA, join('greet', 'SKILL.md'), [
      '---',
      'name: greet',
      'description: A',
      '---',
    ...
    await writeSkill(rootB, join('greet', 'SKILL.md'), [
      '---',
      'name: greet',
      'description: B',
      '---',
  ...
  it('lists flat skills with frontmatter name and description', async () => {
    ...
    await mkdir(root, { recursive: true });
    await writeFile(
      join(root, 'demo-ui-components.md'),
  ...
  it('discovers both flat and subdir skills alongside in the same root', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.scream-code', 'skills');
    await writeSkill(root, join('subdir-skill', 'SKILL.md'), [
      '---',
      'name: subdir-skill',
  ...
  it('prefers the subdir version when a flat and subdir skill share a name', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.scream-code', 'skills');
    await writeSkill(root, join('greet', 'SKILL.md'), [
      '---',
      'name: greet',
      'description: From subdir',
      '---',
  ...
  it('discovers skills nested in subdirectories at multiple depths', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.scream-code', 'skills');
    await writeSkill(root, join('top', 'SKILL.md'), [
      '---',
      'name: top',
      'description: Top level',
      '---',
    ]);
    await writeSkill(root, join('group', 'alpha', 'SKILL.md'), [
      '---',
      'name: alpha',
      'description: One level deep',
      '---',
    ]);
    await writeSkill(root, join('a', 'b', 'beta', 'SKILL.md'), [
      '---',
      'name: beta',
    ...
    await writeFile(join(root, 'group', 'notes.md'), 'just some notes');
  ...
  it('does not descend into a skill bundle subdirectory', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.scream-code', 'skills');
    await writeSkill(root, join('outer', 'SKILL.md'), [
      '---',
      'name: outer',
      'description: The real skill',
      '---',
    ]);
    await writeSkill(root, join('outer', 'references', 'inner', 'SKILL.md'), [
      '---',
      'name: inner',
  ...
  it('skips node_modules when scanning nested directories', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.scream-code', 'skills');
    await writeSkill(root, join('real', 'SKILL.md'), [
      '---',
      'name: real',
      'description: A real skill',
      '---',
    ]);
    await writeSkill(root, join('node_modules', 'pkg', 'vendored', 'SKILL.md'), [
      '---',
      'name: vendored',
  ...
  it('stops recursing past the maximum scan depth', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.scream-code', 'skills');
    await writeSkill(root, join('shallow', 'SKILL.md'), [
      '---',
      'name: shallow',
    ...
    await writeSkill(root, join(...deepSegments, 'deep', 'SKILL.md'), [
      '---',
      'name: deep',
  ...
  it('prefers a shallower skill over a deeper one with the same name', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.scream-code', 'skills');
    await writeSkill(root, join('dup', 'SKILL.md'), [
      '---',
      'name: dup',
      'description: Shallow wins',
      '---',
    ]);
    await writeSkill(root, join('group', 'dup', 'SKILL.md'), [
      '---',
      'name: dup',
  ...
  it('resolves a same-name collision across sibling directories deterministically', async () => {
    ...
    await writeSkill(root, join('group-b', 'dup', 'SKILL.md'), [
      '---',
      'name: dup',
      'description: From group-b',
      '---',
    ]);
    await writeSkill(root, join('group-a', 'dup', 'SKILL.md'), [
      '---',
      'name: dup',
  ...
  it('returns an empty list when the skill root cannot be read', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, 'locked');
    await mkdir(root, { recursive: true });
...
describe('resolveSkillRoots ordering and priority', () => {
  it('returns project then user then builtin in priority order', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const userGeneric = join(homeDir, '.agents', 'skills');
    await mkdir(userGeneric, { recursive: true });
    const projectGeneric = join(repoDir, '.agents', 'skills');
    await mkdir(projectGeneric, { recursive: true });
    const builtin = join(repoDir, 'builtin');
  ...
  it('prefers brand over generic user dirs even when both contain skills with the same name', async () => {
    const { homeDir, workDir } = await makeWorkspace();
    const generic = join(homeDir, '.agents', 'skills');
    await writeSkill(generic, join('greet', 'SKILL.md'), [
      '---',
      'name: greet',
      'description: generic version',
      '---',
    ]);
    const brand = join(homeDir, '.scream-code', 'skills');
    await writeSkill(brand, join('greet', 'SKILL.md'), [
      '---',
      'name: greet',
  ...
  it('returns proj-brand, proj-generic, user-brand, user-generic, builtin in that order without merging', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const userBrand = join(homeDir, '.scream-code', 'skills');
    const userGeneric = join(homeDir, '.agents', 'skills');
    const projBrand = join(repoDir, '.scream-code', 'skills');
    const projGeneric = join(repoDir, '.agents', 'skills');
    const builtin = join(repoDir, 'builtin');
  ...
  it('keeps brand user skills visible when the generic group is empty', async () => {
    const { homeDir, workDir } = await makeWorkspace();
    const generic = join(homeDir, '.config', 'agents', 'skills');
    await mkdir(generic, { recursive: true });
    const brand = join(homeDir, '.scream-code', 'skills');
    await writeSkill(brand, join('deploy', 'SKILL.md'), [
      '---',
      'name: deploy',
  ...
  it('defaults to merging user brand dirs', async () => {
    ...
    expect(paths).toContain(await realpath(join(homeDir, '.scream-code', 'skills')));
  });
});
...
describe('resolveSkillRoots extra dirs', () => {
  it('appends extra dirs to the resolved roots', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const extra = join(repoDir, 'my-extra');
  ...
  it('expands a leading ~/ in extra dirs against the user home directory', async () => {
    const { homeDir, workDir } = await makeWorkspace();
    const target = join(homeDir, 'my-skills');
  ...
  it('resolves a relative extra dir against the project root (.git ancestor), not the work dir', async () => {
    const { homeDir, repoDir } = await makeWorkspace();
    const nested = join(repoDir, 'sub', 'dir');
    await mkdir(nested, { recursive: true });
    const extraAtRoot = join(repoDir, 'my-dir');
    ...
    expect(paths).not.toContain(join(nested, 'my-dir'));
  });

  it('uses absolute extra-dir paths as-is', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const absExtra = join(repoDir, '..', 'somewhere-else');
  ...
  it('silently drops missing extra-dir entries', async () => {
    ...
    const roots = await resolveSkillRoots({
      paths: { userHomeDir: homeDir, workDir },
      extraDirs: [real, join(repoDir, 'nowhere'), join(repoDir, 'nope')],
    });

    const paths = roots.map((r) => r.path);
    expect(paths).toContain(await realpath(real));
    expect(paths).not.toContain(join(repoDir, 'nowhere'));
  });
  ...
  it('keeps plugin-specific skill lookup when a project skill has the same name', async () => {
    const { repoDir } = await makeWorkspace();
    const projectRoot = join(repoDir, '.scream-code', 'skills');
    const pluginRoot = join(repoDir, 'plugin-skills');
    await writeSkill(projectRoot, join('using-superpowers', 'SKILL.md'), [
      '---',
    ...
    await writeSkill(pluginRoot, join('using-superpowers', 'SKILL.md'), [
      '---',
      'name: using-superpowers',
  ...
  it('stamps skills discovered via extra dirs with source=extra', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const extra = join(repoDir, 'my-extra');
    await writeSkill(extra, join('xs', 'SKILL.md'), [
      '---',
      'name: xs',
      'description: x',
      '---',
  ...
  it('combines explicit dirs with extra dirs and suppresses auto-discovery', async () => {
    ...
    const projectBrand = join(repoDir, '.scream-code', 'skills');
    await mkdir(projectBrand, { recursive: true });
    const cli = join(repoDir, 'cli');
    await mkdir(cli, { recursive: true });
    const extra = join(repoDir, 'extra');
    await mkdir(extra, { recursive: true });
  ...
  it('keeps the higher-priority scope when an extra dir overlaps with auto-discovered user dirs', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const userBrand = join(homeDir, '.scream-code', 'skills');
    await writeSkill(userBrand, join('foo', 'SKILL.md'), [
      '---',
      'name: foo',
...
describe('scope priority across resolution and discovery', () => {
  it('lets a user-scope skill win over a same-named builtin', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const builtin = join(repoDir, 'builtin');
    await writeSkill(builtin, join('foo', 'SKILL.md'), [
      '---',
      'name: foo',
  ...
  it('lets a project-scope skill win over a same-named user-scope skill', async () => {
    ...
    const projBrand = join(repoDir, '.scream-code', 'skills');
    await writeSkill(projBrand, join('foo', 'SKILL.md'), [
      '---',
      'name: foo',
  ...
  it('lets an extra-scope skill win over a same-named builtin', async () => {
    ...
    const extra = join(repoDir, 'extra');
    await writeSkill(extra, join('foo', 'SKILL.md'), [
      '---',
      'name: foo',
  ...
  it('fully excludes user and project scopes when explicit dirs are supplied', async () => {
    ...
    const cli = join(repoDir, 'cli');
    await writeSkill(cli, join('bar', 'SKILL.md'), [
      '---',
      'name: bar',
  ...
  it('lets explicit (CLI) skills win over extra skills with the same name', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const cli = join(repoDir, 'cli');
    await writeSkill(cli, join('foo', 'SKILL.md'), [
      '---',
      'name: foo',
      'description: cli version',
      '---',
...
describe('explicit dir override and scope stamping', () => {
  it('suppresses user and project auto-discovery when explicit dirs are present', async () => {
    ...
    const extraA = join(repoDir, 'extra_a');
    const extraB = join(repoDir, 'extra_b');
  ...
  it('stamps each discovered skill with the scope of its root', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const userBrand = join(homeDir, '.scream-code', 'skills');
    await writeSkill(userBrand, join('user-skill', 'SKILL.md'), [
      '---',
      'name: user-skill',
      'description: u',
      '---',
    ...
    const projBrand = join(repoDir, '.scream-code', 'skills');
    await writeSkill(projBrand, join('proj-skill', 'SKILL.md'), [
      '---',
      'name: proj-skill',
      'description: p',
      '---',
...
async function makeWorkspace(): Promise<{
  ...
  const workDir = join(repoDir, 'packages', 'app');
  await mkdir(join(repoDir, '.git'), { recursive: true });
...
describe('project root discovery (.git walk-up)', () => {
  it('walks up to the nearest .git ancestor for project-scope discovery', async () => {
    ...
    const repoScream = join(repo, '.scream-code', 'skills');
    ...
    const nested = join(repo, 'packages', 'sub', 'pkg');
    await mkdir(nested, { recursive: true });
    ...
    expect(projectPaths).toContain(await realpath(repoScream));
  });
  ...
  it('falls back to the work dir when no .git marker is found anywhere up the chain', async () => {
    ...
    await mkdir(join(project, '.scream-code', 'skills'), { recursive: true });
    const workDir = join(project, 'foo');
    await mkdir(workDir, { recursive: true });
    ...
    expect(projectPaths.some((p) => p.includes(join('project', '.scream-code', 'skills')))).toBe(false);
  });
});
