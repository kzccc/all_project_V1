...
describe('Agent turn flow', () => {
  ...
  it('emits a friendly model.not_configured error when no model is configured', async () => {
    ...
    `);
    expect(ctx.newEvents()).toMatchInlineSnapshot(
      `[emit] error   { "code": "model.not_configured", "message": "No model configured. Run \`scream config\` or use \`/model\` to set a default model.", "name": "ScreamError", "details": { "turnId": 0 }, "retryable": false }`,
    );
  });
  ...
  it('does not force continuation for unverified working-set paths', async () => {
    const ctx = testAgent();
    ctx.configure();
    ctx.agent.workingSet.touch('src/foo.ts', 0);
    // Use a non-trivial final response so the summary guard does not fire.
    ctx.mockNextResponse({
      type: 'text',
      text: 'I have completed the requested change to src/foo.ts and it is ready to use.',
    });

    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'Make a change' }] });
    await ctx.untilTurnEnd();

    expect(ctx.llmCalls.length).toBe(1);
    expect(JSON.stringify(ctx.agent.context.data().history)).not.toContain(
      'You have unverified changes in: src/foo.ts',
    );
    expect(JSON.stringify(ctx.agent.context.data().history)).not.toContain('convergence_gate');
  });
  it('continues a turn when the final response is too brief after verification work', async () => {
    const ctx = testAgent();
    ctx.configure();
    ctx.agent.workingSet.recordVerification('pnpm test', '.', 0, 'pass', 0);
    ctx.agent.workingSet.markAllVerified();
    ctx.mockNextResponse({ type: 'text', text: 'Done.' });
    ctx.mockNextResponse({ type: 'text', text: 'Verification passed; src/index.ts is updated.' });

    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'Fix the bug' }] });
    await ctx.untilTurnEnd();
  ...
  it('does not block completion once a failed verification command passes on retry', async () => {
    ...
    const jian = createFakeJian({
      execWithEnv: vi.fn(async () => {
        ...
        if (result === undefined) throw new Error('unexpected extra Bash call');
    ...
    const failingCall: ToolCall = {
      ...
      id: 'call_bash_fail',
      name: 'Bash',
      arguments: JSON.stringify({ command: 'python3 file.py', timeout: 60 }),
    };
    const passingCall: ToolCall = {
      type: 'function',
      id: 'call_bash_pass',
      name: 'Bash',
      arguments: JSON.stringify({ command: 'python3 file.py', timeout: 60 }),
    ...
    ctx.mockNextResponse({ type: 'text', text: 'I will run the verification command.' }, failingCall);
    ctx.mockNextResponse({ type: 'text', text: 'Fix applied; running verification again.' }, passingCall);
    ctx.mockNextResponse({
      type: 'text',
      text: 'Verification passed after the fix. The runtime bugs have been resolved.',
    });

    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'Fix and verify' }] });
    await ctx.untilTurnEnd();

    const history = JSON.stringify(ctx.agent.context.data().history);
    expect(history).not.toContain('A required tool (Bash) failed this turn');
    expect(history).not.toContain('The last verification command failed');
  ...
  it('buffers steer input and includes it in the same turn after approval', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [wire] context.append_loop_event           { "event": { "type": "step.end", "uuid": "<uuid-3>", "turnId": "0", "step": 2, "usage": { "inputOther": 104, "output": 11, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "end_turn" }, "time": "<time>" }
      [emit] turn.step.completed                 { "turnId": 0, "step": 2, "stepId": "<uuid-3>", "usage": { "inputOther": 104, "output": 11, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "end_turn" }
      [wire] usage.record                        { "model": "mock-model", "usage": { "inputOther": 104, "output": 11, "inputCacheRead": 0, "inputCacheCreation": 0 }, "usageScope": "turn", "time": "<time>" }
      [emit] agent.status.updated                { "model": "mock-model", "contextTokens": 115, "maxContextTokens": 1000000, "contextUsage": 0.000115, "planMode": false, "permission": "manual", "usage": { "byModel": { "mock-model": { "inputOther": 111, "output": 33, "inputCacheRead": 0, "inputCacheCreation": 0 } }, "total": { "inputOther": 111, "output": 33, "inputCacheRead": 0, "inputCacheCreation": 0 }, "currentTurn": { "inputOther": 111, "output": 33, "inputCacheRead": 0, "inputCacheCreation": 0 } } }
