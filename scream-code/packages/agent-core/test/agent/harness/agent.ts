...
import { type Environment, type Jian, type JianProcess } from '@scream-cli/jian';
...
export function createCommandJian(stdout: string): Jian {
  function createProcess(): JianProcess {
    return {
...
export class AgentTestContext {
  ...
  configureRuntimeModel(
    ...
    if (this.options.providerManager === undefined) {
      this.screamConfig = configWithProvider(this.screamConfig, provider, modelCapabilities);
    }
...
function createResumeNoSideEffectJian(initialCwd: string): Jian {
  ...
  // `jian.chdir(...)`; let those mutate an internal cwd field so replay
