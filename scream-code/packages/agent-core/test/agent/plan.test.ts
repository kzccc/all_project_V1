...
function createPlanJian(overrides: Parameters<typeof createFakeJian>[0] = {}) {
...
describe('manual plan entry', () => {
  ...
  it('enters plan mode without starting a model turn and prepares the plan directory', async () => {
    ...
    const ctx = testAgent({
      jian: createFakeJian({ mkdir, writeText }),
    });
  ...
  it('derives the no-homedir plan path from cwd on enter and restore', async () => {
    ...
    const resumed = testAgent({ jian: createFakeJian() });
    resumed.dispatch({
...
describe('plan clear', () => {
  it('empties the current plan file without leaving plan mode', async () => {
    ...
    const ctx = testAgent({
      jian: createPlanJian({ mkdir, readText, writeText }),
    });
...
describe('plan exit tool', () => {
  ...
  it('does not execute later tool calls in the same batch after plan rejection', async () => {
    ...
    const ctx = testAgent({
      jian: createPlanJian({ readText, execWithEnv }),
    });
...
describe('plan allows safe tool flow', () => {
  it.each(['Write', 'Edit'] as const)(
    ...
    async (toolName) => {
      ...
      const ctx = testAgent({
        jian: createPlanJian({ readText, writeText }),
      });
  ...
  it('allows read-only Bash to continue through permission and execution', async () => {
    ...
    const ctx = testAgent({ jian: createCommandJian('plan-safe') });
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [wire] context.append_loop_event   { "event": { "type": "step.end", "uuid": "<uuid-3>", "turnId": "0", "step": 2, "usage": { "inputOther": 1155, "output": 12, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "end_turn" }, "time": "<time>" }
      [emit] turn.step.completed         { "turnId": 0, "step": 2, "stepId": "<uuid-3>", "usage": { "inputOther": 1155, "output": 12, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "end_turn" }
      [wire] usage.record                { "model": "mock-model", "usage": { "inputOther": 1155, "output": 12, "inputCacheRead": 0, "inputCacheCreation": 0 }, "usageScope": "turn", "time": "<time>" }
      [emit] agent.status.updated        { "model": "mock-model", "contextTokens": 1167, "maxContextTokens": 1000000, "contextUsage": 0.001167, "planMode": true, "permission": "yolo", "usage": { "byModel": { "mock-model": { "inputOther": 1691, "output": 35, "inputCacheRead": 0, "inputCacheCreation": 0 } }, "total": { "inputOther": 1691, "output": 35, "inputCacheRead": 0, "inputCacheCreation": 0 }, "currentTurn": { "inputOther": 1691, "output": 35, "inputCacheRead": 0, "inputCacheCreation": 0 } } }
...
describe('plan mode Bash ordinary permission behavior', () => {
  it('allows Bash through ordinary yolo permission behavior', async () => {
    ...
    const ctx = testAgent({ jian: createCommandJian('removed') });
    ctx.configure({ tools: ['Bash'] });
    await ctx.rpc.setPermission({ mode: 'yolo' });
    await ctx.agent.planMode.enter('test-plan', false);
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [wire] context.append_loop_event   { "event": { "type": "step.end", "uuid": "<uuid-3>", "turnId": "0", "step": 2, "usage": { "inputOther": 1151, "output": 9, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "end_turn" }, "time": "<time>" }
      [emit] turn.step.completed         { "turnId": 0, "step": 2, "stepId": "<uuid-3>", "usage": { "inputOther": 1151, "output": 9, "inputCacheRead": 0, "inputCacheCreation": 0 }, "finishReason": "end_turn" }
      [wire] usage.record                { "model": "mock-model", "usage": { "inputOther": 1151, "output": 9, "inputCacheRead": 0, "inputCacheCreation": 0 }, "usageScope": "turn", "time": "<time>" }
      [emit] agent.status.updated        { "model": "mock-model", "contextTokens": 1160, "maxContextTokens": 1000000, "contextUsage": 0.00116, "planMode": true, "permission": "yolo", "usage": { "byModel": { "mock-model": { "inputOther": 1684, "output": 32, "inputCacheRead": 0, "inputCacheCreation": 0 } }, "total": { "inputOther": 1684, "output": 32, "inputCacheRead": 0, "inputCacheCreation": 0 }, "currentTurn": { "inputOther": 1684, "output": 32, "inputCacheRead": 0, "inputCacheCreation": 0 } } }
      [emit] turn.ended                  { "turnId": 0, "reason": "completed" }
    `);
