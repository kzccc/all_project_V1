...
function expectConfigInvalid(fn: () => unknown): void {
  try {
    ...
    expect((error as ScreamError).code).toBe('config.invalid');
    return;
  }
...
const MIN = {
  SCREAM_MODEL_NAME: 'scream-for-coding',
  SCREAM_MODEL_API_KEY: 'sk-test',
  SCREAM_MODEL_MAX_CONTEXT_SIZE: '262144',
} as const;

describe('applyEnvModelConfig', () => {
  it('returns the config unchanged when SCREAM_MODEL_NAME is absent', () => {
    const base = getDefaultConfig();
    expect(applyEnvModelConfig(base, {})).toBe(base);
  });

  it('throws when SCREAM_MODEL_NAME is set but API key is missing', () => {
    expectConfigInvalid(() => apply({ SCREAM_MODEL_NAME: 'm' }));
  });

  it('defaults max_context_size to 262144 (256K) when unset', () => {
    expect(
      apply({ SCREAM_MODEL_NAME: 'm', SCREAM_MODEL_API_KEY: 'k' })
  ...
  it.each(['abc', '0', '1.5', '-1'])(
    ...
    (value) => {
      expectConfigInvalid(() =>
        apply({ ...MIN, SCREAM_MODEL_MAX_CONTEXT_SIZE: value }),
      );
    },
  );
  ...
  it('applies provider type and its default base url', () => {
    expect(apply({ ...MIN, SCREAM_MODEL_PROVIDER_TYPE: 'openai' })
      .providers[ENV_MODEL_PROVIDER_KEY]).toMatchObject({
      type: 'openai',
      baseUrl: 'https://api.openai.com/v1',
    });
    const anthropic = apply({ ...MIN, SCREAM_MODEL_PROVIDER_TYPE: 'anthropic' })
  ...
  it('rejects unsupported provider types', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, SCREAM_MODEL_PROVIDER_TYPE: 'google-genai' }),
    );
  });

  it('lets an explicit base url override the default', () => {
    expect(
      apply({ ...MIN, SCREAM_MODEL_BASE_URL: 'https://api-staff.msh.team/v1' })
  ...
  it('parses comma-separated capabilities (trimmed, lowercased)', () => {
    expect(
      apply({ ...MIN, SCREAM_MODEL_CAPABILITIES: 'Image_In, thinking ,' })
  ...
  it('sets display_name only when provided', () => {
    ...
    expect(
      apply({ ...MIN, SCREAM_MODEL_DISPLAY_NAME: 'Staff Model' })
  ...
  it('writes type-specific fields and validates max_output_size', () => {
    const alias = apply({
      ...MIN,
      SCREAM_MODEL_PROVIDER_TYPE: 'anthropic',
      SCREAM_MODEL_MAX_OUTPUT_SIZE: '8192',
      SCREAM_MODEL_REASONING_KEY: 'reasoning',
    }).models?.[ENV_MODEL_ALIAS_KEY];
    expect(alias?.maxOutputSize).toBe(8192);
    expect(alias?.reasoningKey).toBe('reasoning');
    expectConfigInvalid(() =>
      apply({ ...MIN, SCREAM_MODEL_MAX_OUTPUT_SIZE: 'nope' }),
    );
  });
  ...
  it('maps the thinking variables', () => {
    const config = apply({
      ...MIN,
      SCREAM_MODEL_DEFAULT_THINKING: 'true',
      SCREAM_MODEL_THINKING_MODE: 'on',
      SCREAM_MODEL_THINKING_EFFORT: 'high',
    });
    expect(config.defaultThinking).toBe(true);
    expect(config.thinking).toMatchObject({ mode: 'on', effort: 'high' });
    expect(apply({ ...MIN, SCREAM_MODEL_DEFAULT_THINKING: '0' }).defaultThinking)
      .toBe(false);
  });

  it('rejects an invalid thinking mode', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, SCREAM_MODEL_THINKING_MODE: 'bogus' }),
    );
  });

  it('maps SCREAM_MODEL_ADAPTIVE_THINKING onto the alias', () => {
    expect(
      apply({ ...MIN, SCREAM_MODEL_ADAPTIVE_THINKING: 'true' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(true);
    expect(
      apply({ ...MIN, SCREAM_MODEL_ADAPTIVE_THINKING: 'false' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(false);
    expect(
  ...
  it('rejects an invalid SCREAM_MODEL_ADAPTIVE_THINKING', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, SCREAM_MODEL_ADAPTIVE_THINKING: 'maybe' }),
    );
  });
...
describe('stripEnvModelConfig (write-back guard)', () => {
  ...
  it('keeps user providers/models and a non-env default_model', () => {
    ...
    config.providers['scream'] = { type: 'scream', apiKey: 'k', baseUrl: 'https://x/v1' };
    config.providers[ENV_MODEL_PROVIDER_KEY] = { type: 'scream', apiKey: 'env-key' };
    config.models = {
      'my-model': { provider: 'scream', model: 'm', maxContextSize: 1000 },
    ...
    expect(stripped.providers['scream']).toBeDefined();
...
describe('writeConfigFile never persists the env model', () => {
  ...
  it('output never contains env-injected identifiers', async () => {
    ...
    try {
      const runtime = loadRuntimeConfig(path, {
        ...MIN,
        SCREAM_MODEL_THINKING_EFFORT: 'low',
        SCREAM_MODEL_DEFAULT_THINKING: 'true',
      });
      await writeConfigFile(path, runtime);
...
describe('SCREAM_MODEL_DEFAULT_THINKING validation', () => {
  it('rejects a non-empty unparseable value', () => {
    expectConfigInvalid(() => apply({ ...MIN, SCREAM_MODEL_DEFAULT_THINKING: 'flase' }));
  });

  it('accepts valid values and ignores when unset', () => {
    expect(apply({ ...MIN, SCREAM_MODEL_DEFAULT_THINKING: 'true' }).defaultThinking).toBe(true);
    expect(apply({ ...MIN, SCREAM_MODEL_DEFAULT_THINKING: '0' }).defaultThinking).toBe(false);
