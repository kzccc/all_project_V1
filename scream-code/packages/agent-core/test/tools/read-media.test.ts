...
import type { ContentPart, ModelCapability } from '@scream-cli/ltod';
...
function makeReadMediaTool(
  input: {
    readonly stat?: Jian['stat'] | undefined;
    readonly readBytes?: Jian['readBytes'] | undefined;
    readonly modelCapabilities?: ModelCapability | undefined;
  } = {},
): ReadMediaFileTool {
  const jian = createFakeJian({
    stat: input.stat ?? vi.fn<Jian['stat']>().mockResolvedValue(DEFAULT_STAT),
    readBytes: input.readBytes ?? vi.fn<Jian['readBytes']>().mockResolvedValue(PNG_HEADER),
  });
...
describe('ReadMediaFileTool', () => {
  ...
  it('returns a system/text/image/text wrap for PNG files', async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi.fn<Jian['stat']>().mockResolvedValue({ ...DEFAULT_STAT, stSize: data.length }),
      readBytes: vi.fn<Jian['readBytes']>().mockResolvedValue(data),
    });
  ...
  it('expands leading tilde paths using the jian home directory', async () => {
    const data = Buffer.concat([PNG_HEADER, Buffer.from('pngdata')]);
    const readBytes = vi.fn<Jian['readBytes']>().mockResolvedValue(data);
    const tool = makeReadMediaTool({
      stat: vi.fn<Jian['stat']>().mockResolvedValue({ ...DEFAULT_STAT, stSize: data.length }),
      readBytes,
    });
  ...
  it('rejects text files with a Read hint', async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi.fn<Jian['stat']>().mockResolvedValue({ ...DEFAULT_STAT, stSize: text.length }),
      readBytes: vi.fn<Jian['readBytes']>().mockResolvedValue(text),
    });
  ...
  it('rejects unknown binary files without legacy Python-tool wording', async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi.fn<Jian['stat']>().mockResolvedValue({ ...DEFAULT_STAT, stSize: blob.length }),
      readBytes: vi.fn<Jian['readBytes']>().mockResolvedValue(blob),
    });
  ...
  it('rejects empty files and files exceeding the media size limit', async () => {
    const empty = await executeTool(makeReadMediaTool({
      stat: vi.fn<Jian['stat']>().mockResolvedValue({ ...DEFAULT_STAT, stSize: 0 }),
    }), {
      turnId: 't1',
      toolCallId: 'c_empty',
  ...
  it('exposes a <system> summary with the original pixel size for sized images', async () => {
    ...
    const tool = makeReadMediaTool({
      stat: vi.fn<Jian['stat']>().mockResolvedValue({ ...DEFAULT_STAT, stSize: png.length }),
      readBytes: vi.fn<Jian['readBytes']>().mockResolvedValue(png),
    });
  ...
  it('description by capabilities lockdown — image + video points at Read for text fallback', () => {
    const tool = new ReadMediaFileTool(createFakeJian(), PERMISSIVE_WORKSPACE, capabilities());
  ...
  it('allows absolute media paths outside workspace but rejects relative escapes', async () => {
    const readBytes = vi.fn<Jian['readBytes']>().mockResolvedValue(PNG_HEADER);
    const tool = new ReadMediaFileTool(
      createFakeJian({
        stat: vi.fn<Jian['stat']>().mockResolvedValue(DEFAULT_STAT),
        readBytes,
      }),
