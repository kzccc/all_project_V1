...
import type { JianProcess, StatResult } from '@scream-cli/jian';
...
import { createFakeJian, toolContentString } from './fixtures/fake-jian';
...
function processWithOutput(stdout: string, stderr = '', exitCode = 0): JianProcess {
  return {
...
function processThatExitsOnKill(stdout: string, stderr = '', exitCode = 143): JianProcess {
...
describe('GrepTool', () => {
  ...
  it('sorts files_with_matches by mtime before pagination after sensitive filtering', async () => {
    ...
    const tool = new GrepTool(
      createFakeJian({ exec: vi.fn().mockResolvedValue(processWithOutput(stdout)), stat }),
  ...
  it('uses the backend path class when filtering sensitive grep results', async () => {
    ...
    const exec = vi.fn().mockResolvedValue(processWithOutput(stdout));
    const tool = new GrepTool(createFakeJian({ exec, pathClass: () => 'win32' }), {
  ...
  it('applies offset and head_limit after rg output is collected', async () => {
    ...
    const tool = new GrepTool(
      createFakeJian({ exec: vi.fn().mockResolvedValue(processWithOutput(stdout)) }),
      { workspaceDir: '/workspace', additionalDirs: [] },
    );
  ...
  it('returns generic spawn errors from jian.exec', async () => {
  ...
  it('maps schema flags onto ripgrep equivalents and tilde-expands ~ in path', async () => {
    ...
    const homeTool = new GrepTool(
      createFakeJian({ exec, gethome: () => '/home/test' }),
  ...
  it('strips workspace prefix from rg output across record kinds (win32)', async () => {
    ...
    const exec = vi.fn().mockResolvedValue(processWithOutput(stdout));
    const tool = new GrepTool(
      createFakeJian({ exec, pathClass: () => 'win32' }),
  ...
  it('treats a trailing-slash workspace dir the same as one without', async () => {
    const withSep = new GrepTool(
      createFakeJian({ exec: vi.fn().mockResolvedValue(processWithOutput('/tmp/dir/file.py\n')) }),
