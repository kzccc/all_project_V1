...
describe('ScreamCliFetchURLProvider caching', () => {
  it('returns a cached result on the second fetch', async () => {
    ...
    const first = await provider.fetch('https://example.com/page');
    const second = await provider.fetch('https://example.com/page');

    expect(first).toEqual({ content: 'service content', kind: 'extracted' });
    expect(second).toBe(first);
    expect(fetchImpl).toHaveBeenCalledTimes(1);
  });

  it('caches fallback results from the local fetcher', async () => {
    const getAccessToken = vi.fn<() => Promise<string>>().mockResolvedValue('token');
    ...
    expect(first).toEqual({ content: 'fallback content', kind: 'passthrough' });
    expect(second).toBe(first);
    expect(fetchImpl).toHaveBeenCalledTimes(1);
