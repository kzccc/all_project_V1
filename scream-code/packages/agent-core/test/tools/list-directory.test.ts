...
describe('listDirectory', () => {
  ...
  it('uses the backend path class when reading child directories', async () => {
    ...
    const tree = await listDirectory(jian, 'C:\\workspace');
  ...
  it('returns "(empty directory)" when the dir has no entries', async () => {
    const jian = createFakeJian({
      ...
      iterdir: async function* (_p: string) {} as unknown as Jian['iterdir'],
    });
    const result = await listDirectory(jian, '/empty');
  ...
  it('returns [not readable] when the root directory itself is inaccessible', async () => {
    ...
    } as Parameters<typeof createFakeJian>[0]);
    const result = await listDirectory(jian, '/no-access');
