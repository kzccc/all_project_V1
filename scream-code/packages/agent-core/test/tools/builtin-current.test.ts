...
function processWithOutput(stdout: string, exitCode = 0): JianProcess {
  return {
...
describe('current builtin file and shell tools', () => {
  it('Read exposes parameters and reads text content', async () => {
    ...
    const tool = new ReadTool(
      createFakeJian({
        stat: vi.fn<Jian['stat']>().mockResolvedValue(regularFileStat),
        readBytes: vi.fn<Jian['readBytes']>().mockImplementation(async (_path, n) => {
          return n === undefined ? bytes : bytes.subarray(0, n);
        }),
        readLines: vi.fn<Jian['readLines']>().mockImplementation(async function* readLines() {
          yield 'alpha\n';
          yield 'beta\n';
        }),
      }),
    ...
    expect(result.output).toBe(
      [
        '1\talpha',
        '2\tbeta',
        '<system>2 lines read from file starting from line 1. Total lines in file: 2. End of file reached. Anchor: e49c81e2</system>',
      ].join('\n'),
    );
  });

  it('Write exposes parameters and writes through jian', async () => {
    const writeText = vi.fn().mockResolvedValue(5);
    const tool = new WriteTool(
      createFakeJian({ writeText, stat: vi.fn<Jian['stat']>().mockResolvedValue(directoryStat) }),
      workspace,
    );
  ...
  it('Edit exposes parameters and errors when old_string is missing', async () => {
    const tool = new EditTool(
      createFakeJian({ readText: vi.fn().mockResolvedValue('alpha\nbeta\n') }),
      workspace,
    );
  ...
  it('Grep exposes parameters and rejects relative workspace escapes before spawning rg', async () => {
    const jian = createFakeJian({ exec: vi.fn() });
    const tool = new GrepTool(jian, workspace);
    ...
    expect(jian.exec).not.toHaveBeenCalled();
  });
