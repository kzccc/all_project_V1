/**
 * Fake Jian — minimal stub for tool constructor injection in tests.
 *
...
import type { Environment, Jian } from '@scream-cli/jian';
import type { ExecutableToolResult } from '#/loop';

import type { WorkspaceConfig } from '../../../src/tools/support/workspace';

function notImplemented(method: string): never {
  throw new Error(`FakeJian.${method} not implemented — override in test`);
}
...
export function createFakeJian(overrides?: Partial<Jian>): Jian {
  // Hold cwd in a closure so `chdir` (which `config.update({cwd})` now
  // routes through) can mutate it and later `getcwd()` calls see the
  // update — mirroring real-jian semantics without needing a backing fs.
  ...
  const base: Jian = {
    ...
    gethome: () => '/home/test',
    getcwd: () => cwd,
    withCwd: (next: string) => createFakeJian({ ...overrides, getcwd: () => next }),
