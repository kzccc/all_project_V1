...
describe('McpConnectionManager', () => {
  ...
  it('reconnect throws a coded ScreamError when the server name is unknown', async () => {
    const cm = new McpConnectionManager();
    try {
      await expect(cm.reconnect('nope')).rejects.toBeInstanceOf(ScreamError);
  ...
  it('reconnect rejects disabled servers without connecting them', async () => {
    ...
    try {
      ...
      await expect(reconnect).rejects.toBeInstanceOf(ScreamError);
...
describe('Session MCP startup', () => {
  it('stores default MCP OAuth credentials under the configured Scream home', async () => {
    ...
    const screamHome = join(tmp, 'scream-home');
    ...
    try {
      ...
      await expect(
        readFile(
          join(processHome, '.scream-code', 'credentials', 'mcp', `${provider.storeKey}-tokens.json`),
          'utf-8',
        ),
