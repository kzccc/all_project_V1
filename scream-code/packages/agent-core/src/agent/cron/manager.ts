...
export class CronManager {
  ...
  constructor(agent: Agent, opts: CronManagerOptions = {}) {
    ...
    this.clocks =
      opts.clocks ??
      resolveClockSources(process.env['SCREAM_CRON_CLOCK']) ??
      SYSTEM_CLOCKS;
    this.persistStore =
    ...
    this.scheduler = createCronScheduler({
      ...
      // P1.8: `SCREAM_CRON_MANUAL_TICK=1` forces the scheduler into
      ...
      pollIntervalMs:
        process.env['SCREAM_CRON_MANUAL_TICK'] === '1'
          ? null
          : opts.pollIntervalMs,
    });
   ...
   * Stale judgment.
   *
   *   - `SCREAM_CRON_NO_STALE=1` short-circuits to false (bench).
  ...
  isStale(task: CronTask): boolean {
    if (process.env['SCREAM_CRON_NO_STALE'] === '1') return false;
  ...
  private handleFire(
    ...
    void this.agent.turn.steer(content, origin);
   ...
   *
   * Gated on `SCREAM_CRON_MANUAL_TICK=1` for two reasons:
   *
  ...
  private bindSigusr1(): void {
    ...
    if (process.env['SCREAM_CRON_MANUAL_TICK'] !== '1') return;
