...
class RootLoggerImpl implements RootLogger {
  ...
  attachSession(input: SessionAttachInput): SessionLogHandle {
    ...
    const sink = new RotatingFileSink({
      path: join(input.sessionDir, 'logs', 'scream-code.log'),
 ...
 *
 *   import { log } from 'scream-code-sdk';
 *   log.info('scream-code starting', { version });
 *
 ...
 * importing `log` at module load (before `ScreamHarness` configures the root)
...
export function resolveGlobalLogPath(homeDir: string): string {
  return join(homeDir, 'logs', 'scream-code.log');
}
