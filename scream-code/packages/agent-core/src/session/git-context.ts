 ...
 */
export async function collectGitContext(jian: Jian, cwd: string): Promise<string> {
  // Quick check: is this a git repo?
  if ((await runGit(jian, cwd, ['rev-parse', '--is-inside-work-tree'])) === null) {
    return '';
  }

  const [remoteUrl, branch, dirtyRaw, logRaw] = await Promise.all([
    runGit(jian, cwd, ['remote', 'get-url', 'origin']),
    runGit(jian, cwd, ['branch', '--show-current']),
    runGit(jian, cwd, ['status', '--porcelain']),
    runGit(jian, cwd, ['log', '-3', '--format=%h %s']),
  ]);
 ...
 * `git -C` form runs in the target directory regardless of the Jian backend.
 */
async function runGit(jian: Jian, cwd: string, args: readonly string[]): Promise<string | null> {
  let proc;
  try {
    proc = await jian.exec('git', '-C', cwd, ...args);
  } catch {
    return null;
  }
