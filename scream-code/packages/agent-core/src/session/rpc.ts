...
export class SessionAPIImpl implements PromisableMethods<SessionAPI> {
  ...
  private getAgent(agentId: string): PromisableMethods<AgentAPI> {
    ...
    if (agent === undefined) {
      throw new ScreamError(ErrorCodes.AGENT_NOT_FOUND, `Agent "${agentId}" was not found`);
    }
  ...
  async injectPlugin(_payload: InjectPluginPayload): Promise<void> {
    // Implemented directly on ScreamCore so it can access the plugin manager.
    throw new ScreamError(
      ErrorCodes.NOT_IMPLEMENTED,
      'injectPlugin is implemented at the core level',
    );
  }
}
