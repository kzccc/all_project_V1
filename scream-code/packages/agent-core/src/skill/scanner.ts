...
const USER_BRAND_DIRS = ['.scream-code/skills'] as const;
const USER_GENERIC_DIRS = ['.agents/skills'] as const;
const PROJECT_BRAND_DIRS = ['.scream-code/skills'] as const;
...
export async function discoverSkills(
  ...
  async function walkSkillDir(
    ...
    for (const entry of entries) {
      ...
      if (await isDir(join(dirPath, entry))) subdirs.push(entry);
    }
    ...
    if (isTopLevel) {
      ...
      for (const entry of entries) {
        ...
        if (directorySkills.has(skillName)) {
          warn(
            `Ignoring flat skill ${join(dirPath, entry)} because ${join(dirPath, skillName, 'SKILL.md')} exists with the same name`,
          );
          continue;
        }
    ...
    for (const entry of subdirs) {
      await walkSkillDir(join(dirPath, entry), root, false, depth + 1);
    }
  }
...
async function pushFirstExisting(
  ...
  for (const dir of dirs) {
    if (await pushExistingRoot(out, join(base, dir), source, isDir, realpath)) return;
  }
}
...
async function pushBrandGroup(
  ...
  for (const dir of dirs) {
    await pushExistingRoot(out, join(base, dir), source, isDir, realpath);
  }
}
