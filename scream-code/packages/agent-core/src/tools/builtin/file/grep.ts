 ...
 *
 * Shells out to `rg` through Jian. Supports glob/type filtering, context
 * lines, output modes, pagination, multiline, and case-insensitive search.
 *
 * Path safety is enforced before any Jian I/O. Explicit absolute paths outside
...
export class GrepTool implements BuiltinTool<GrepInput> {
  ...
  private async execution(
    ...
    let runResult = await runRipgrepOnce(this.jian, buildRgArgs(rgPath, args, searchPaths), signal);
    ...
    try {
      orderedLines =
        mode === 'files_with_matches' && !timedOut
          ? await sortFilesWithMatchesByMtime(keptLines, this.jian, signal)
          : keptLines;
    } catch (error) {
...
async function sortFilesWithMatchesByMtime(
  ...
  const entries = await mapWithConcurrency(
    ...
    async (line, index) => {
      ...
      if (path !== undefined) {
        try {
          mtime = (await jian.stat(path)).stMtime ?? 0;
        } catch {
