...
# Runtime settings for Scream Code.
 ...
 * synthesized from `SCREAM_MODEL_*` environment variables. Use this everywhere a
...
export function parseConfigString(tomlText: string, filePath = 'config.toml'): ScreamConfig {
  ...
  try {
    ...
    throw new ScreamError(ErrorCodes.CONFIG_INVALID, `Invalid TOML in ${filePath}: ${error instanceof Error ? error.message : String(error)}`, {
      cause: error,
    });
  }
...
function parseConfigData(data: Record<string, unknown>, filePath: string): ScreamConfig {
  ...
  try {
    ...
    throw new ScreamError(ErrorCodes.CONFIG_INVALID, `Invalid configuration in ${filePath}: ${formatConfigValidationError(error)}`, {
      cause: error,
    });
  }
}
