...
export const LoopControlSchema = z.object({
  ...
  // Hard ceiling on the number of turns a single goal-mode run may consume.
  // Only enforced when the model has not set an explicit turn budget; it
  // prevents runaway loops where the model neither completes nor blocks the
  // goal. Set to 0 to disable the default guard.
...
export const McpServerStdioConfigSchema = z.object({
  ...
  // Reserved for future jian-backed stdio launchers. `undefined` and `'local'`
  // both mean direct child_process spawn for now.
  executor: z.enum(['local', 'jian']).optional(),
...
export function validateConfig(config: unknown): ScreamConfig {
  try {
    ...
    throw new ScreamError(ErrorCodes.CONFIG_INVALID, `Invalid configuration: ${formatConfigValidationError(error)}`, {
      cause: error,
    });
  }
}
