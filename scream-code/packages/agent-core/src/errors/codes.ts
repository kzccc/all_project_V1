...
export type ScreamErrorCode = (typeof ErrorCodes)[keyof typeof ErrorCodes];

export interface ScreamErrorInfo {
...
export const SCREAM_ERROR_INFO = {
  'config.invalid': {
  ...
  'skill.already_exists': {
    ...
    retryable: false,
    public: true,
    action: 'Choose a different skill name or delete the existing skill first.',
  },
  'skill.install_failed': {
    title: 'Skill installation failed',
    retryable: false,
    public: true,
    action: 'Check disk space, permissions, and the skill package contents.',
  },
  ...
  'plugin.load_failed': {
    ...
    retryable: true,
    public: true,
    action: 'Fix the installed.json file under $SCREAM_CODE_HOME/plugins/ and run /plugins reload.',
  },
  ...
  'shell.git_bash_not_found': {
    ...
    retryable: false,
    public: true,
    action: 'Install Git for Windows from https://gitforwindows.org/ or set SCREAM_SHELL_PATH to a bash.exe.',
  },
...
} as const satisfies Record<ScreamErrorCode, ScreamErrorInfo>;
