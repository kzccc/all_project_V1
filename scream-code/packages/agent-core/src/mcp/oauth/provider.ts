 ...
 *    state under `<SCREAM_CODE_HOME>/credentials/mcp/<key>-*.json`
 *    (mode 0600; default home is `~/.scream-code`).
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("scream-code (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `scream-code (${options.serverName})`;
  }
