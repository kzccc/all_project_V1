...
export interface McpOAuthServiceOptions {
  /** Storage backend; overrides `screamHomeDir` when supplied. */
  readonly store?: JsonFileStore;
  /** Resolved Scream home; credentials default to `<screamHomeDir>/credentials/mcp/`. */
...
export class McpOAuthService {
  ...
  constructor(options: McpOAuthServiceOptions = {}) {
    this.store =
      options.store ??
      new JsonFileStore(
        options.screamHomeDir === undefined ? undefined : mcpCredentialsDir(options.screamHomeDir),
      );
