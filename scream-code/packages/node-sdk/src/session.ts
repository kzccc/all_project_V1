import { ErrorCodes, ScreamError, type AgentContextData, type ScreamErrorCode } from '@scream-cli/agent-core';
...
export class Session {
   ...
   */
  async injectPlugin(id: string): Promise<void> {
    this.ensureOpen();
    const normalized = normalizeRequiredString(
      id,
    ...
    await this.rpc.injectPlugin({ sessionId: this.id, id: normalized });
  }
  /**
  ...
  async removeSkill(skillName: string): Promise<void> {
    ...
    await this.rpc.removeSkill({ sessionId: this.id, skillName: normalized });
  }
  ...
  async updateGoalStatus(status: 'active' | 'complete' | 'paused' | 'blocked'): Promise<GoalSnapshotData | null> {
    this.ensureOpen();
  ...
  private ensureOpen(): void {
    if (this.closed) {
      throw new ScreamError(ErrorCodes.SESSION_CLOSED, 'Session is closed');
    }
  }
}
...
function normalizeRequiredString(
  ...
  if (normalized.length === 0) {
    throw new ScreamError(code, message);
  }
  return normalized;
}
