...
import type { Event, ScreamError, SkillActivatedEvent, SkillSummary } from '#/index';
...
vi.mock('@scream-cli/ltod', async (importOriginal) => {
...
const { ScreamHarness, Session } = await import('#/index');
...
describe('Session skills', () => {
  it('lists session skills without exposing content', async () => {
    ...
    try {
      ...
      expect(listed?.path.endsWith('/.scream-code/skills/review/SKILL.md')).toBe(true);
  ...
  it('resolves user skills from the OS home directory, independently of SCREAM_CODE_HOME', async () => {
    const homeDir = await makeTempDir(tempDirs, 'scream-sdk-skills-home-');
    ...
    const harness = new ScreamHarness({ identity: TEST_IDENTITY });
