...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'scream-sdk-export-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('exportSessionDirectory', () => {
  ...
  it('omits global log manifest path when the global log cannot be bundled', async () => {
    ...
    expect(result.entries).not.toContain('logs/global/scream-code.log');
    const entries = readZipEntries(await readFile(outputPath));
...
describe('ScreamHarness.exportSession', () => {
  ...
  it('rejects missing session ids', async () => {
    ...
    await expect(missingExport).rejects.toBeInstanceOf(ScreamError);
