...
import { ScreamError, ScreamHarness, type Event } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'scream-sdk-rename-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('ScreamHarness.renameSession', () => {
  ...
  it('rejects missing session ids', async () => {
    ...
    try {
      ...
      await expect(missingRename).rejects.toBeInstanceOf(ScreamError);
