...
import { ScreamError, ScreamHarness } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'scream-sdk-config-'));
  tempDirs.push(dir);
  return dir;
}

const COMPLETE_TOML = `
default_model = "scream-for-coding"
...
[providers.scream-for-coding]
type = "scream"
base_url = "https://api.scream.com/coding/v1"
api_key = "sk-xxx"
custom_headers = { "X-Custom-Header" = "value" }

[providers.scream-for-coding.env]
GOOGLE_CLOUD_PROJECT = "project-1"

[models.scream-for-coding]
provider = "scream-for-coding"
...
describe('SDK config TOML', () => {
  it('parses the documented config shape and keeps TUI-only fields in raw', () => {
    ...
    expect(config.defaultModel).toBe('scream-for-coding');
    ...
    const provider = config.providers['scream-for-coding'];
    ...
    expect(config.models?.['scream-for-coding']).toMatchObject({
      provider: 'scream-for-coding',
      model: 'scream-for-coding',
  ...
  it('writes typed fields in snake_case and preserves unknown raw sections', async () => {
    ...
    await writeConfigFile(configPath, {
      ...config,
      defaultModel: 'scream-for-coding',
      loopControl: {
        ...config.loopControl,
        maxStepsPerTurn: 42,
      },
    ...
    expect(text).toContain('default_model = "scream-for-coding"');
...
describe('ScreamHarness config API', () => {
  it('loads default config when missing and deep-merges setConfig patches from disk', async () => {
    ...
    expect(config.providers['scream-for-coding']).toMatchObject({
      type: 'scream',
      baseUrl: 'https://api.scream.com/coding/v1',
      apiKey: 'sk-updated',
      env: { GOOGLE_CLOUD_PROJECT: 'project-1' },
  ...
  it('does not write invalid config patches', async () => {
    ...
    await expect(setInvalidConfig).rejects.toBeInstanceOf(ScreamError);
