...
// ── E2E: exec edge cases ──────────────────────────────────────────────
//
// Covers exec() scenarios that the other jian suites do not touch:
//
...
//   - proving that each LocalJian instance carries its OWN cwd into
...
describe('e2e: exec edge cases', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'jian-exec-edge-')));
  ...
  describe('spawning a non-existent command', () => {
    it('exec() rejects promptly when the binary does not exist (never hangs)', async () => {
      // Contract: LocalJian.exec() awaits the child's 'spawn' or 'error' event
      // before returning, so a missing binary becomes a synchronous rejection
      // rather than a ghost process handle.
      await expect(jian.exec('this-binary-does-not-exist-jian-edge-test-12345')).rejects.toThrow(
  ...
  describe('cwd isolation for concurrent instances', () => {
    it('two LocalJian instances with different cwds run concurrent child processes that each see their own cwd', async () => {
      const subA = join(tempDir, 'A');
      const subB = join(tempDir, 'B');

      await jian.mkdir(subA);
      await jian.mkdir(subB);

      const jianA = await LocalJian.create();
      const jianB = await LocalJian.create();
      await jianA.chdir(subA);
      await jianB.chdir(subB);
      ...
      const [procA, procB] = await Promise.all([
        jianA.exec('node', '-e', 'process.stdout.write(process.cwd())'),
        jianB.exec('node', '-e', 'process.stdout.write(process.cwd())'),
      ]);
      ...
      // Each child's cwd MUST equal its jian instance's cwd.
    ...
    it('execWithEnv honors the per-instance cwd and injects env vars', async () => {
      const proc = await jian.execWithEnv(
        ['node', '-e', 'process.stdout.write(process.env.JIAN_TEST_MARKER + "|" + process.cwd())'],
        { JIAN_TEST_MARKER: 'beacon42', PATH: process.env['PATH'] ?? '' },
      );
      const stdout = await streamToString(proc.stdout);
      const exitCode = await proc.wait();
