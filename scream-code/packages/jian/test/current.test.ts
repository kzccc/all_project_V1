...
describe('getCurrentJian', () => {
  it('returns the LocalJian bound by the test setup', () => {
    const jian = getCurrentJian();
    expect(jian).toBeInstanceOf(LocalJian);
    expect(jian.name).toBe('local');
  });
});

describe('module-level proxy functions', () => {
  it('normpath delegates to the current jian instance', () => {
    // LocalJian on posix normalizes '/foo/../bar' to '/bar'
    const result = normpath('/foo/../bar');
    expect(typeof result).toBe('string');
    expect(result.endsWith('bar')).toBe(true);
  });

  it('pathClass returns posix or win32 from the current jian', () => {
    const result = pathClass();
    expect(result === 'posix' || result === 'win32').toBe(true);
  });

  it('readLines proxies to the current jian and yields lines', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'jian-readlines-'));
    try {
  ...
  it('writeText accepts an encoding option through the module-level proxy', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'jian-writetext-enc-'));
    try {
