...
describe('LocalJian', () => {
  ...
  describe('pathClass, gethome, getcwd', () => {
    ...
    it('should return the home directory', () => {
      // Python test_local_jian.py pins `str(gethome()) == str(Path.home())`;
  ...
  describe('chdir + stat', () => {
    it('should change directory and stat a file', async () => {
      ...
      await jian.mkdir(nested);

      await jian.chdir(nested);
      expect(jian.getcwd()).toBe(nested);

      const filePath = join(nested, 'file.txt');
      await jian.writeText(filePath, 'hello world');

      const statResult = await jian.stat(filePath);
    ...
    it('should reject chdir into a regular file', async () => {
      ...
      await expect(jian.chdir(filePath)).rejects.toThrow(/Not a directory/);
    });
  ...
  describe('iterdir path normalization', () => {
    it('should produce normalized paths even when the argument has a trailing separator', async () => {
      ...
      // separator.
      await jian.writeText(join(tempDir, 'file.txt'), 'x');
  ...
  describe('iterdir + glob', () => {
    it('should list directory entries and match glob patterns', async () => {
      await jian.mkdir(join(tempDir, 'alpha'));
  ...
  describe('glob hidden files', () => {
    it('should include hidden files in glob results', async () => {
      await jian.writeText(join(tempDir, '.gitlab-ci.yml'), 'stages: [build]');
      await jian.writeText(join(tempDir, 'config.yml'), 'key: value');

      const matched: string[] = [];
      for await (const entry of jian.glob(tempDir, '*.yml')) {
        matched.push(entry);
      }
    ...
    it('should glob through hidden directories with ** pattern', async () => {
      await jian.mkdir(join(tempDir, 'src'));
      await jian.mkdir(join(tempDir, 'src', '.config'));
      await jian.writeText(join(tempDir, 'src', '.config', 'settings.yml'), 'debug: true');
      await jian.writeText(join(tempDir, 'src', 'main.ts'), 'pass');

      const deepMatched: string[] = [];
      for await (const entry of jian.glob(tempDir, 'src/**/*.yml')) {
  ...
  describe('readText/writeText', () => {
    it('should write, read, append, and readLines', async () => {
      ...
      const written = await jian.writeText(filePath, 'line1');
  ...
  describe('readText errors parameter (Python compat)', () => {
    ...
    it('throws on invalid utf-8 with errors="strict" (default)', async () => {
      ...
      await expect(jian.readText(filePath)).rejects.toThrow();
      await expect(jian.readText(filePath, { errors: 'strict' })).rejects.toThrow();
    });
    ...
    it('returns U+FFFD replacement characters with errors="replace"', async () => {
      ...
      const content = await jian.readText(filePath, { errors: 'replace' });
    ...
    it('drops invalid bytes with errors="ignore"', async () => {
      ...
      const content = await jian.readText(filePath, { errors: 'ignore' });
  ...
  describe('LF preservation', () => {
    it('should not convert LF to CRLF', async () => {
      ...
      const raw = await jian.readBytes(filePath);
  ...
  describe('mkdir recursive', () => {
    it('should create nested directories with parents option', async () => {
      ...
      await jian.mkdir(nested, { parents: true });

      const s = await jian.stat(nested);
    ...
    it('should throw when parents:true + existOk:false on existing dir', async () => {
      ...
      await jian.mkdir(existing);

      await expect(jian.mkdir(existing, { parents: true, existOk: false })).rejects.toThrow();
    });
    ...
    it('should succeed when parents:true + existOk:true on existing dir', async () => {
      ...
      await expect(jian.mkdir(existing, { parents: true, existOk: true })).resolves.toBeUndefined();
    });
    ...
    it('should throw when existOk:true but conflicting path is a file', async () => {
      ...
      await expect(jian.mkdir(filePath, { existOk: true })).rejects.toBeInstanceOf(
        JianFileExistsError,
      );
    });
  });
  ...
  describe('glob character class negation', () => {
    it('[!a] should match non-a files (glob negation, not literal `!`)', async () => {
      ...
      await jian.writeText(join(tempDir, 'a.txt'), '');
      await jian.writeText(join(tempDir, 'b.txt'), '');
      await jian.writeText(join(tempDir, '!.txt'), '');

      const matches: string[] = [];
      for await (const m of jian.glob(tempDir, '[!a].txt')) {
        matches.push(m);
      }
  ...
  describe('glob ** pattern', () => {
    it('should not return duplicates when ** matches nested paths', async () => {
      ...
      await jian.mkdir(join(tempDir, 'a', 'b'), { parents: true });
      await jian.writeText(join(tempDir, 'a', 'b', 'file.txt'), 'content');

      const matches: string[] = [];
      for await (const m of jian.glob(tempDir, '**/*.txt')) {
        matches.push(m);
      }
    ...
    it('should match ** at multiple depths without duplicates', async () => {
      await jian.mkdir(join(tempDir, 'a', 'b', 'c'), { parents: true });
      await jian.writeText(join(tempDir, 'root.txt'), '');
      await jian.writeText(join(tempDir, 'a', 'mid.txt'), '');
      await jian.writeText(join(tempDir, 'a', 'b', 'c', 'deep.txt'), '');
    ...
    it('should not duplicate matches for deep ** patterns', async () => {
      ...
      await jian.mkdir(join(tempDir, 'l1', 'l2', 'l3', 'l4'), { parents: true });
      await jian.writeText(join(tempDir, 'l1', 'l2', 'l3', 'l4', 'deep.txt'), 'x');
    ...
    it('should yield basePath and every recursive entry when the whole pattern is **', async () => {
      ...
      await jian.mkdir(join(tempDir, 'sub'));
      await jian.writeText(join(tempDir, 'root.txt'), 'r');
      await jian.writeText(join(tempDir, 'sub', 'nested.txt'), 'n');

      const matches: string[] = [];
      for await (const m of jian.glob(tempDir, '**')) {
        matches.push(m);
      }
  ...
  describe('glob symlink cycle safety', () => {
    ...
    it('T-C5 regression — non-symlink tree results are unchanged', async () => {
      ...
      await jian.mkdir(join(tempDir, 'a', 'b', 'c'), { parents: true });
      await jian.writeText(join(tempDir, 'r1.txt'), '');
      await jian.writeText(join(tempDir, 'a', 'r2.txt'), '');
      await jian.writeText(join(tempDir, 'a', 'b', 'c', 'r3.txt'), '');
  ...
  describe('readBytes/writeBytes', () => {
    it('should round-trip binary data', async () => {
      ...
      const written = await jian.writeBytes(filePath, data);
      expect(written).toBe(4);

      const read = await jian.readBytes(filePath);
  ...
  describe('exec streaming', () => {
    it('should run a command and stream stdout/stderr', async () => {
      ...
      const proc = await jian.exec(...nodeArgs(code));
  ...
  describe('exec non-zero exit', () => {
    it('should return the correct exit code', async () => {
      const proc = await jian.exec(...nodeArgs('process.exit(7)'));
      const exitCode = await proc.wait();
  ...
  describe('exec spawn failure', () => {
    it('should reject when the binary does not exist', async () => {
      await expect(jian.exec('/absolutely/non-existent/binary')).rejects.toThrow();
    });
    ...
    it('should reject exec() with no arguments', async () => {
      ...
      await expect((jian.exec as () => Promise<unknown>)()).rejects.toThrow(
    ...
    it('should reject execWithEnv() with an empty args array', async () => {
      ...
      await expect(jian.execWithEnv([])).rejects.toThrow(/at least one argument/);
    });
  });
...
describe('LocalJian instance isolation', () => {
  test('instances have isolated cwds (no process.cwd pollution)', async () => {
    const jianA = await LocalJian.create();
    const jianB = await LocalJian.create();

    const tmpA = await realpath(await mkdtemp(join(tmpdir(), 'jian-a-')));
    const tmpB = await realpath(await mkdtemp(join(tmpdir(), 'jian-b-')));

    try {
      await jianA.chdir(tmpA);
      await jianB.chdir(tmpB);

      // jianA.chdir must not affect jianB's cwd (no process.chdir pollution).
      expect(jianA.getcwd()).toBe(tmpA);
      expect(jianB.getcwd()).toBe(tmpB);

      // Write a file named "marker.txt" in each cwd using a relative path.
      await jianA.writeText('marker.txt', 'A');
      await jianB.writeText('marker.txt', 'B');

      // Read back via each jian — each should get its own version.
      expect(await jianA.readText('marker.txt')).toBe('A');
      expect(await jianB.readText('marker.txt')).toBe('B');

      // exec() should also honour the instance cwd.
      const procA = await jianA.exec('node', '-e', 'process.stdout.write(process.cwd())');
      const procB = await jianB.exec('node', '-e', 'process.stdout.write(process.cwd())');
      await procA.wait();
      await procB.wait();
...
describe('LocalProcess.kill safety', () => {
  test('kill() is safe when spawn failed (pid -1 must not signal process group)', async () => {
    ...
    let proc;
    try {
      proc = await jian.exec('this-command-does-not-exist-xyz123');
    } catch {
  ...
  test.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const jian = await LocalJian.create();
      const tmp = await realpath(await mkdtemp(join(tmpdir(), 'jian-killtree-posix-')));
      try {
...
describe('LocalJian process environment allowlist', () => {
  let jian: LocalJian;
  ...
  it('inherits PATH but strips secrets from spawned processes', async () => {
    vi.stubEnv('OPENAI_API_KEY', 'should-not-leak');
    try {
      const proc = await jian.exec('node', '-e', 'console.log(process.env.OPENAI_API_KEY ?? "missing")');
  ...
  it('preserves explicit env values passed by the caller', async () => {
    const proc = await jian.execWithEnv(
      ['node', '-e', 'console.log(process.env.CALLER_VALUE ?? "missing")'],
      { CALLER_VALUE: 'present' },
    );
    const output = await streamToString(proc.stdout);
    await proc.wait();
    expect(output.trim()).toBe('present');
  });
});

describe('LocalJian optional root sandbox', () => {
  let tempDir: string;
  let sandboxDir: string;

  beforeEach(async () => {
    tempDir = await realpath(await mkdtemp(join(tmpdir(), 'jian-sandbox-')));
  ...
  it('rejects file operations outside the configured root', async () => {
    const jian = await LocalJian.create(sandboxDir, sandboxDir);
    const outside = join(tempDir, 'outside.txt');
    await expect(jian.writeText(outside, 'x')).rejects.toThrow('Path outside allowed root directory');
  });

  it('allows file operations inside the configured root', async () => {
    const jian = await LocalJian.create(sandboxDir, sandboxDir);
    const inside = join(sandboxDir, 'inside.txt');
    await expect(jian.writeText(inside, 'x')).resolves.toBeDefined();
    expect(await jian.readText(inside)).toBe('x');
  });

  it('preserves the root boundary across withCwd()', async () => {
    const jian = await LocalJian.create(sandboxDir, sandboxDir);
    const subDir = join(sandboxDir, 'sub');
    await mkdir(subDir);
    const narrowed = jian.withCwd(subDir);
    const outside = join(tempDir, 'outside.txt');
