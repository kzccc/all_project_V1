 ...
 * Since the new Jian.exec(...args) doesn't take options, timeout is implemented
...
async function runSh(
  ...
  const proc: JianProcess = await jian.exec('/bin/sh', '-c', command);
...
describe.skipIf(process.platform === 'win32')('LocalJian shell operations', () => {
  let jian: Jian;
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'jian-shell-'));
  ...
  it('should run a simple command', async () => {
    const result = await runSh(jian, "echo 'Hello World'");
  ...
  it('should handle command with error', async () => {
    const result = await runSh(jian, 'ls /nonexistent/directory');
  ...
  it('should support command chaining with &&', async () => {
    const result = await runSh(jian, "echo 'First' && echo 'Second'");
    expect(result.exitCode).toBe(0);
  ...
  it('should support command pipe', async () => {
    const result = await runSh(jian, "echo 'Hello World' | wc -w");
    expect(result.exitCode).toBe(0);
  ...
  it('should handle command with timeout (completes before timeout)', async () => {
    ...
    const result = await runSh(jian, 'sleep 0.1', { timeout: 5000 });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toBe('');
    expect(result.stderr).toBe('');
  });
  ...
  it('should handle timeout expiration', async () => {
    ...
    const result = await runSh(jian, 'sleep 60', { timeout: 100 });
    expect(result.exitCode).toBe(-1);
  });

  it('should pass environment variables to shell', async () => {
    const result = await runSh(jian, 'TEST_VAR=\'test_value\'; export TEST_VAR; echo "$TEST_VAR"');
    expect(result.exitCode).toBe(0);
  ...
  it('should perform file operations', async () => {
    // Mirror Python test_file_operations: two separate jian.exec calls so
    ...
    const write = await runSh(jian, `echo 'Test content' > "${filePath}"`);
    ...
    const read = await runSh(jian, `cat "${filePath}"`);
  ...
  it('should handle stdin data', async () => {
    ...
    const result = await runSh(jian, 'read value; printf \'%s\\n\' "$value"', {
  ...
  it('should execute commands sequentially with ;', async () => {
    const result = await runSh(jian, "echo 'One'; echo 'Two'");
    expect(result.exitCode).toBe(0);
  ...
  it('should support conditional execution with ||', async () => {
    const result = await runSh(jian, "false || echo 'Success'");
    expect(result.exitCode).toBe(0);
  ...
  it('should support multiple pipes', async () => {
    const result = await runSh(jian, "printf '1\\n2\\n3\\n' | grep '2' | wc -l");
    expect(result.exitCode).toBe(0);
  ...
  it('should handle text processing with sed', async () => {
    const result = await runSh(jian, "echo 'apple banana cherry' | sed 's/banana/orange/'");
    expect(result.exitCode).toBe(0);
  ...
  it('should support command substitution', async () => {
    const result = await runSh(jian, 'echo "Result: $(echo hello)"');
    expect(result.exitCode).toBe(0);
  ...
  it('should support arithmetic substitution', async () => {
    const result = await runSh(jian, 'echo "Answer: $((2 + 2))"');
    expect(result.exitCode).toBe(0);
  ...
  it('should handle very long output', async () => {
    const result = await runSh(jian, 'seq 1 100 | head -50');
    expect(result.exitCode).toBe(0);
