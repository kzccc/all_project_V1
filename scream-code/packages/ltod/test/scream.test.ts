...
import { extractUsageFromChunk, ScreamChatProvider } from '#/providers/scream';
...
function createProvider(stream: boolean = false): ScreamChatProvider {
  return new ScreamChatProvider({
...
function getGenerationState(provider: ScreamChatProvider): ScreamGenerationState {
...
async function captureRequestBody(
  ...
  (provider as any)._client.chat.completions.create = vi
    ...
    .mockImplementation((params: unknown) => {
      ...
      return Promise.resolve(makeChatCompletionResponse('scream-k2'));
    });
...
describe('ScreamChatProvider', () => {
  ...
  describe('provider properties', () => {
    it('has correct name and model', () => {
      const provider = createProvider();
      expect(provider.name).toBe('scream');
      expect(provider.modelName).toBe('scream-k2-turbo-preview');
    });

    it('throws during generation when no constructor or request API key is provided', async () => {
      // Save and clear env var
      const saved = process.env['SCREAM_API_KEY'];
      delete process.env['SCREAM_API_KEY'];
      try {
        const provider = new ScreamChatProvider({ model: 'test' });
        await expect(provider.generate('', [], [])).rejects.toThrow(/options\.auth\.apiKey/);
      } finally {
        if (saved !== undefined) {
          process.env['SCREAM_API_KEY'] = saved;
        }
      }
    });
    ...
    it('passes request-scoped auth to the client factory', async () => {
      ...
      const client = {
        chat: {
          completions: {
            create: vi.fn().mockResolvedValue(makeChatCompletionResponse('scream-k2')),
          },
        },
      };
      const provider = new ScreamChatProvider({
    ...
    it('withThinking returns a new instance', () => {
      ...
      expect(newProvider).toBeInstanceOf(ScreamChatProvider);
  ...
  describe('clone client sharing', () => {
    ...
    function getInternalClient(provider: ScreamChatProvider): unknown {
