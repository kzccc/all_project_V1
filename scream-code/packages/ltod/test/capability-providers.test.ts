...
import { ScreamChatProvider } from '#/providers/scream';
...
describe('ScreamChatProvider.getCapability', () => {
  function make(model: string): ScreamChatProvider {
    return new ScreamChatProvider({ model, apiKey: 'test-key' });
  }
  ...
  it('does not infer capabilities from Scream model names', () => {
    for (const model of [
      'scream-for-coding',
      'scream-code',
      'scream-k2-turbo-preview',
      'scream-k2.5',
      'scream-thinking-preview',
    ]) {
  ...
  it('explicit model arg overrides this.modelName', () => {
    const provider = make('scream-k2-turbo-preview');
    expect(provider.getCapability('scream-for-coding')).toEqual(UNKNOWN_CAPABILITY);
  });
