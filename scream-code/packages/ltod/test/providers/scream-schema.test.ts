import { derefJsonSchema, normalizeScreamToolSchema } from '#/providers/scream-schema';
...
describe('normalizeScreamToolSchema', () => {
  it.each([
    {
      name: 'string enum',
    ...
    ({ property, expectedType }) => {
      ...
      const result = normalizeScreamToolSchema(schema);
  ...
  it('fails fast for mixed enum types instead of emitting an unsupported Scream type array', () => {
    ...
    expect(() => normalizeScreamToolSchema(schema)).toThrow(
  ...
  it('does not default the root schema itself to string', () => {
    expect(normalizeScreamToolSchema({})).toEqual({});
  });
