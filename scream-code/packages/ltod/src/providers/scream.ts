...
import { normalizeScreamToolSchema } from './scream-schema';
...
import { ScreamFiles } from './scream-files';
import {
...
export interface ScreamOptions {
...
function convertTool(tool: Tool): OpenAIToolParam {
  ...
  return {
    ...
    function: {
      ...converted.function,
      parameters: normalizeScreamToolSchema(tool.parameters),
    },
  };
}
/**
...
class ScreamStreamedMessage implements StreamedMessage {
...
export class ScreamChatProvider implements ChatProvider {
  readonly name: string = 'scream';
   ...
   * messages.
   */
  get files(): ScreamFiles {
    this._files ??= new ScreamFiles({
      apiKey: this._apiKey,
      baseUrl: this._baseUrl,
      defaultHeaders: this._defaultHeaders,
  ...
  async generate(
    ...
    try {
      ...
      return new ScreamStreamedMessage(response, this._stream);
    } catch (error: unknown) {
      throw convertOpenAIError(error);
    }
  }
  ...
  withGenerationKwargs(kwargs: GenerationKwargs): ScreamChatProvider {
    return this._withGenerationKwargs(kwargs);
  }

  withMaxCompletionTokens(maxCompletionTokens: number): ScreamChatProvider {
    return this._withGenerationKwargs({ max_completion_tokens: maxCompletionTokens });
  }

  withExtraBody(extraBody: ExtraBody): ScreamChatProvider {
  ...
  private _withGenerationKwargs(kwargs: GenerationKwargs): ScreamChatProvider {
    const clone = this._clone();
    clone._generationKwargs = { ...clone._generationKwargs, ...kwargs };
    return clone;
  }

  private _clone(): ScreamChatProvider {
    const clone = Object.assign(
      Object.create(Object.getPrototypeOf(this) as object) as ScreamChatProvider,
      this,
    );
    clone._generationKwargs = { ...this._generationKwargs };
    // Do not share the memoized ScreamFiles instance with the clone; let it be
    ...
    // construction; instead build a new ScreamChatProvider when a real new
    // client is required.
    return clone;
  }
}
