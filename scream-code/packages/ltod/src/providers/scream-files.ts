 ...
 *
 * A `ScreamFiles` instance is typically obtained from
 * {@link ScreamChatProvider.files}.
 */
export class ScreamFiles {
  ...
  constructor(options: ScreamFilesOptions) {
    ...
    if (typeof input === 'string') {
      ...
      if (mimeType === undefined || !mimeType.startsWith('video/')) {
        throw new ChatProviderError(
          `ScreamFiles.uploadVideo: file extension does not indicate a video type: ${filename}`,
        );
      }
