...
import { OldSessionStateSchema, type OldSessionState } from '../scream-cli-schema.js';
...
export async function migrateOneSession(input: MigrateOneInput): Promise<MigrateOneResult> {
  ...
  if (existsSync(targetDir)) {
    ...
    // A real, unrelated scream-code session occupies the path — a true conflict.
    if (cls === 'foreign') {
 ...
 *  - `foreign`:  a real, unrelated scream-code session occupying the path.
