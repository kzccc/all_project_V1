...
import { OldScreamJsonSchema, OldSessionStateSchema } from '../scream-cli-schema.js';
...
export async function migrateSessionsStep(
  ...
  for (const wd of workdirs) {
    md5ToWorkdir.set(oldMd5BucketName(wd.path), { path: wd.path, jian: wd.jian });
  }

  let bucketsScanned = 0;
  let bucketsSkippedNonlocalJian = 0;
  ...
  for (const bucketName of bucketDirs) {
    ...
    if (workdir.kind === 'nonlocal-jian') {
      bucketsSkippedNonlocalJian++;
      continue;
    }
...
function resolveBucket(
  ...
  if (MD5_HEX_RE.test(bucketName)) {
    ...
    if (meta.jian !== 'local') {
      return { kind: 'nonlocal-jian' };
    }
    return { kind: 'local', path: meta.path };
  }
  // Non-local pattern: `<jian>_<md5>`. Use the last `_` so jian names that
