/**
 * Plan describing the contents detected under the source `~/.scream/` directory.
...
/**
 * One workdir bucket (`~/.scream/sessions/<md5>/`) with reverse-looked-up
 * path from `~/.scream/scream.json`. Buckets with jian != 'local' or no
 * workdir found are excluded from this list (they appear in counters).
 */
export interface WorkDirEntry {
  readonly oldHashDir: string; // absolute path to ~/.scream/sessions/<md5>/
 ...
 * `~/.scream-code/migration-report.json` and surfaced in the terminal summary.
 */
...
export interface MigrationSummary {
  ...
  readonly mcp: {
    ...
    /** Source servers dropped because scream-code's MCP schema rejects them. */
...
export interface SessionsSummary {
  ...
  readonly bucketsSkippedNonlocalJian: number;
...
export interface MigrationNotices {
  ...
  /**
   * Basenames of scream-cli OAuth logins (`~/.scream/credentials/<name>.json`)
   ...
   * scream-code instead. Empty when the legacy install had no OAuth login.
   */
