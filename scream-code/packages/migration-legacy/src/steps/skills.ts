...
/**
 * Copy the user's legacy skills tree (~/.scream/skills/) into scream-code's
 * default user skills root (~/.scream-code/skills/). Granularity is one
...
export async function migrateSkillsStep(input: SkillsStepInput): Promise<SkillsStepResult> {
  ...
  for (const name of entries) {
    ...
    // in it — touching it earlier would fail when ~/.scream-code/skills is
