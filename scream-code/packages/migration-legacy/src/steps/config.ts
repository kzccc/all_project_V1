...
// scream-code's tui.toml `theme` enum (mirrors apps/scream-code TuiThemeSchema).
...
// The config.toml top-level keys scream-code understands, derived from the live
// ScreamConfigSchema so the set tracks scream-code automatically. `raw` is internal
...
export interface ConfigStepResult {
  ...
  /** Top-level keys dropped because scream-code's config schema lacks them. */
  ...
  /** Count of scream-cli hook entries dropped because scream-code's schema rejects them. */
...
/** True when the scream-cli provider entry validates against scream-code's schema. */
...
/** True when the scream-cli model entry validates against scream-code's schema. */
...
export async function migrateConfigStep(input: ConfigStepInput): Promise<ConfigStepResult> {
  ...
  // 1) Providers — keep only those scream-code's schema accepts.
  ...
  // 2) Models — keep only those scream-code's schema accepts, and not those
  ...
  // 2b) Hooks — keep only entries scream-code's HookDefSchema accepts. scream-cli
  //     and scream-code share an identical hook shape, so a valid legacy hook
  //     passes straight through; the per-entry filter only guards against
  //     future schema drift (an event type / field scream-code does not know).
  ...
  // 4) Build the migrated top-level — only keys scream-code's schema supports.
  ...
  for (const [k, v] of Object.entries(parsed)) {
    ...
    if (k === 'default_yolo') {
      // scream-cli's `default_yolo` maps to scream-code's `default_permission_mode`.
  ...
  // 4b) Drop any supported top-level key whose VALUE scream-code's config
  //     schema rejects (e.g. `yolo = "false"`, `extra_skill_dirs = "/tmp"`).
