...
[providers."managed:scream-code".oauth]
storage = "file"
...
describe('migrateConfigStep', () => {
  ...
  it('additively merges into a user-modified target config', async () => {
    ...
    expect(cfg).toContain('yolo = true'); // additively brought over
    expect(cfg).toContain('scream-code/scream-for-coding'); // migrated model added
  });
...
`,
    ...
    const r = await migrateConfigStep({ sourceHome: src, targetHome: tgt });
    expect(r.configConflicts).toContain('providers.managed:scream-code');
    const cfg = await readFile(join(tgt, 'config.toml'), 'utf-8');
    expect(cfg).toContain('https://target.example/v1');
    expect(cfg).not.toContain('https://source.example/v1');
  });

  it('drops top-level keys scream-code does not support', async () => {
    await writeFile(
      join(src, 'config.toml'),
  ...
  it('falls back to a sibling file when the target config is unparseable', async () => {
    ...
    expect(
      await readFile(join(tgt, 'config.migrated-from-scream-cli.toml'), 'utf-8'),
  ...
  it('a tui.toml-only conflict sets wroteTuiSibling, not wroteSiblingDueToConflict', async () => {
    ...
    expect(
      await readFile(join(tgt, 'tui.migrated-from-scream-cli.toml'), 'utf-8'),
    ).toContain('theme');
    // original kept
  ...
  it('drops a kept-provider model missing required schema fields', async () => {
    // `bad-model` references the kept `managed:scream-code` provider but omits
    // `max_context_size`, which scream-code's ModelAliasSchema requires. Written
    // verbatim it would make getConfig() reject the whole config post-migration.
    const cfg = `[providers."managed:scream-code"]
type = "scream"
base_url = "https://api.scream.com/coding/v1"
...
`;
  ...
  it('drops a supported top-level key whose value the schema rejects', async () => {
    await writeFile(join(src, 'config.toml'), 'yolo = "false"\ndefault_thinking = true\n');
    await writeFile(join(tgt, 'config.toml'), DEFAULT_CONFIG_FILE_TEXT);
    const r = await migrateConfigStep({ sourceHome: src, targetHome: tgt });
    expect(r.migrated).toBe(true);
    // `yolo` is a supported key, but the string "false" is not a boolean
    // — writing it verbatim would make the next getConfig() reject the file.
    expect(r.droppedKeys).toContain('yolo');
    const cfg = await readFile(join(tgt, 'config.toml'), 'utf-8');
    expect(cfg).not.toContain('yolo');
    expect(cfg).toContain('default_thinking');
  });
...
`,
  ...
  it('drops a legacy theme outside the scream-code TUI enum', async () => {
  ...
  it('drops a single hook scream-code\'s schema rejects, keeps the rest', async () => {
    await writeFile(
      join(src, 'config.toml'),
      '[[hooks]]\n' +
        'event = "PreToolUse"\n' +
  ...
  it('reports migratedHooks=0 and populates siblingContents when sibling mode kicks in', async () => {
    ...
    // `config.migrated-from-scream-cli.toml`. Hooks land in the sibling, NOT in
