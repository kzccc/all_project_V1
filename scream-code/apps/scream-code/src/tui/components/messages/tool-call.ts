...
import type { TokenUsage } from '@scream-cli/scream-code-sdk';
...
function extractKeyArgument(
  ...
  // Glob: concatenate pattern, optional path, and include_dirs override
  // into a single summary so the header shows `*.ts · src · no dirs`.
  if (toolName === 'Glob') {
...
export class ToolCallComponent extends Container {
   ...
   * `args.plan` is empty. `ScreamTUI` calls `setPlanInfo` with
  ...
  // when ScreamTUI routes a `subagent.event` with this tool call
   ...
   * this: plan-file mode leaves `args.plan` empty, so `ScreamTUI` fetches
  ...
  // ── Subagent API (called by ScreamTUI event routing) ───────────────
