 ...
 *
 * scream-core's LLM pipeline only accepts PNG/JPEG/GIF/WebP, and the
...
function readVideoPath(path: string): ClipboardVideo | null {
  ...
  if (stat.size > MAX_VIDEO_BYTES) {
    throw new ClipboardMediaError(
      `视频大小为 ${(stat.size / 1024 / 1024).toFixed(1)} MB；最大支持 100 MB。`,
    );
  }
  return {
    kind: 'video',
...
function readClipboardImageViaPowerShell(): ClipboardImage | null {
  const tmpFile = join(tmpdir(), `scream-wsl-clip-${randomUUID()}.png`);
  try {
    ...
    const psScript = [
      ...
      '$path = $env:SCREAM_WSL_CLIPBOARD_IMAGE_PATH',
    ...
    const result = runCommand('powershell.exe', ['-NoProfile', '-Command', psScript], {
      ...
      env: { ...process.env, SCREAM_WSL_CLIPBOARD_IMAGE_PATH: winPath },
    });
