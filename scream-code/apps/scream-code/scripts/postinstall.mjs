 ...
 * installs a fresh `scream` shim into the global bin dir; this script
 * removes any pre-existing `scream` shim left behind by the previous
 ...
 *   - Does not touch a `scream` we don't recognize as the previous
 ...
 *     `scream`-vs-`scream-legacy` classification, the rename / unlink
 *     primitives.
 ...
 *      install a `scream` shim under its global bin directory.
 ...
 *      touch becomes `scream-legacy` (preserves what `scream` referred
 ...
 *      we delete `scream` anyway (still achieves takeover) and tell
...
async function main() {
  ...
  //     needed)
  //   - an unrelated `scream` we don't recognize (a user's own wrapper
  ...
  // `scream` used to refer to). Every subsequent shim is just
