 ...
 *     `scream-legacy`. The "`scream` now launches the new CLI" claim is
 ...
 *     slot was a user-managed file; we deleted the source `scream` to
...
export function logMigrationDone(outcomes, pm) {
  ...
  if (renames.length > 0) {
    lines.push(pad('   Renamed your old scream so you can still run it as'));
  ...
  if (consolidates.length > 0) {
    lines.push(pad('   Removed an extra copy of your old scream (scream-legacy'));
  ...
  if (skippedForeignTarget.length > 0) {
    lines.push(pad('   Removed your old scream (a file you created was already'));
  ...
  if (deletes.length > 0) {
    ...
    lines.push(pad('   new scream if we left them):'));
  ...
  if (blockedHarmless.length > 0) {
    ...
    lines.push(pad('   they won\'t run instead of the new scream:'));
...
export function logMigrationBlocked(blocked, actionable, pm) {
  ...
  const lines = [
    ...
    '',
    pad('   There\'s an old scream on your computer that we can\'t change.'),
    pad('   As long as it\'s there, typing `scream` will still run the old'),
  ...
  if (actionable.length > 0) {
    lines.push(
      '',
      pad('   We also found these old scream files. We could remove them'),
...
/**
 * The reachability check found a `scream` ahead of our shim on PATH
...
export function logForeignScreamInTheWay(foreignPath, pm) {
  ...
  emit(
    renderBox([
      warningHeading('Can\'t switch to the new scream yet'),
      '',
      pad('   There\'s another file called `scream` on your computer that\'s'),
      pad('   not the new CLI and not the old one — it looks like'),
      pad('   something you set up yourself. As long as it\'s there,'),
      pad('   typing `scream` will run it instead of the new version.'),
      '',
      pad('   We found it at:'),
      pathInBox(foreignPath),
      '',
      pad('   To use the new scream, delete or rename that file, then'),
      pad('   install again:'),
      pathInBox(reinstallCmd),
      '',
      pad('   Nothing on your computer was changed.'),
...
/**
 * The legacy `scream` was found, but the directory where the package
 * manager placed the new `scream` shim is not on the user's PATH.
 * Renaming the legacy shim now would leave the user with NO reachable
 * `scream` command — the new one would still not be discoverable by
...
export function logNewCliNotOnPath(detection, pm) {
  ...
  emit(
    renderBox([
      ...
      '',
      pad('   The old scream is still here:'),
      pathInBox(detection.shimPath),
      '',
      pad('   The new scream was installed by ' + pm + ', but it landed in a'),
      pad('   folder your terminal doesn\'t search. (Your terminal looks'),
      pad('   for commands in folders listed in your PATH.) If we removed'),
      pad('   the old scream now, typing `scream` wouldn\'t find anything.'),
      '',
      pad('   Add the new scream\'s folder to your PATH (and save the change'),
      pad('   in your ' + rcLabel + ' so it sticks), then install again:'),
      pathInBox(newPathHint),
      pathInBox(reinstallCmd),
      '',
      pad('   The old scream is still where it was.'),
    ]),
  );
}
