/**
 * Where the user actually types things, and what `scream` resolves to
 * from there.
 *
 * Covers:
 *
 ...
 *     `scream` into, not just the installer's environment.
 ...
 *     unknown/foreign `scream` (e.g. a user-written wrapper).
 *
...
/**
 * Expand a basename like `scream` into the set of filenames the OS
 * would actually match on PATH.
 *
 * On POSIX: just `['scream']`.
 *
 * On Windows: `['scream', 'scream.exe', 'scream.cmd', …]` — every
 ...
 * `scream` into. A `~/.zshrc`-only PATH entry, a `~/.bash_profile`
 ...
 *     because either source may contain a legacy `scream` we want to
 ...
 *     the user will actually type `scream` into. We do NOT union here:
