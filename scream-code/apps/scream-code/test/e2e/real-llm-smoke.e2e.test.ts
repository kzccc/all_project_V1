/**
 * Real-LLM smoke. Opt-in only — runs when `SCREAM_E2E_REAL=1` is set, so
 * `make test` / CI never touches the network. Goes through the SDK's
 * `ScreamHarness` entry and round-trips one real prompt. Requires provider
 * credentials on disk.
 *
 * Env knobs:
 *   SCREAM_E2E_REAL     — set to "1" to enable this suite
 *   SCREAM_E2E_MODEL    — model alias override (default: config's default)
 *   SCREAM_E2E_PROMPT   — prompt text (default: "Reply with a single word: hi")
 *   SCREAM_E2E_WORKDIR  — workspace directory (default: /tmp/scream-e2e)
 */

import { mkdirSync } from 'node:fs';
import process from 'node:process';

import { ScreamHarness, type Event } from '@scream-cli/scream-code-sdk';
import { describe, expect, test } from 'vitest';

import { createScreamCodeHostIdentity, getVersion } from '#/cli/version';

const DEFAULT_PROMPT = 'Reply with a single word: hi';
const DEFAULT_WORKDIR = '/tmp/scream-e2e';
const TURN_TIMEOUT_MS = 60_000;

const ENABLED = process.env['SCREAM_E2E_REAL'] === '1';
type TurnEndedEvent = Extract<Event, { readonly type: 'turn.ended' }>;

describe.skipIf(!ENABLED)('SDK e2e — real LLM smoke', () => {
  test(
    'round-trips a single prompt through ScreamHarness',
    async () => {
      const workDir = process.env['SCREAM_E2E_WORKDIR'] ?? DEFAULT_WORKDIR;
      const prompt = process.env['SCREAM_E2E_PROMPT'] ?? DEFAULT_PROMPT;
      const modelAlias = process.env['SCREAM_E2E_MODEL'];
