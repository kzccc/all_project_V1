...
import { ScreamHarness, log } from '@scream-cli/scream-code-sdk';
import { __resetRootLoggerForTest } from '../../../../packages/agent-core/src/logging/logger';

const SESSION_LOG = 'logs/scream-code.log';
const GLOBAL_LOG = 'logs/global/scream-code.log';
const MAIN_WIRE = 'agents/main/wire.jsonl';
const ENABLED = process.env['SCREAM_E2E'] === '1';
...
beforeEach(async () => {
  ...
  homeDir = await mkdtemp(join(tmpdir(), 'scream-cli-log-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'scream-cli-log-work-'));
  oldHome = process.env['SCREAM_CODE_HOME'];
...
afterEach(async () => {
  ...
  if (oldHome === undefined) {
    ...
    process.env['SCREAM_CODE_HOME'] = oldHome;
  }
...
async function runScreamExport(args: string[]): Promise<void> {
  ...
  registerExportCommand(program, {
    ...
    exit: (code: number): never => {
      throw new Error(`scream export exited ${code}: ${stderr.join('')}`);
    },
  });
  await program.parseAsync(['node', 'scream', 'export', ...args]);
}
