...
const SERVER_ENTRY = join(REPO_ROOT, 'plugins/official/scream-datasource/bin/scream-datasource.mjs');

describe.skip('scream-datasource MCP server', () => {
  it('exposes the same two generic tools as the Python plugin', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'scream-datasource-plugin-'));
    const screamHome = join(tempDir, 'scream-home');
    let child: ChildProcessWithoutNullStreams | undefined;

    try {
      await mkdir(join(screamHome, 'credentials'), { recursive: true });
      await writeFile(
        join(screamHome, 'credentials', 'scream-code.json'),
