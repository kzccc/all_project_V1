...
import type { ContentPart, ToolCall } from '@scream-cli/scream-code-sdk';
import type { ContextMessage, PromptOrigin } from '@scream-cli/scream-code-sdk';
...
describe('formatToolCallMd', () => {
  ...
  it('renders tool call without hint', () => {
    ...
    const md = formatToolCallMd(tc);
    expect(md).toContain('#### 工具调用: CustomTool');
...
describe('buildExportMarkdown', () => {
  ...
  it('builds complete markdown with frontmatter and overview', () => {
    ...
    expect(md).toContain('# Scream 会话导出');
    expect(md).toContain('## 概览');
    expect(md).toContain('Hello world');
    expect(md).toContain('## 轮次 1');
  ...
  it('counts turns correctly in overview', () => {
    ...
    expect(md).toContain('3 轮');
    expect(md).toContain('## 轮次 1');
    expect(md).toContain('## 轮次 2');
