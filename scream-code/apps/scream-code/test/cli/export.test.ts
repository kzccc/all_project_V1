...
const mocks = vi.hoisted(() => ({
  ...
  resolveScreamHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/scream-export-home'),
  ...
  return {
    ...
    ScreamHarness: class {
      ...
      constructor(...args: unknown[]) {
        ...
        this.homeDir = options?.homeDir ?? '/tmp/scream-export-home';
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'scream-export-'));
});
...
afterEach(() => {
  ...
  mocks.resolveScreamHome.mockImplementation(
    (homeDir?: string) => homeDir ?? '/tmp/scream-export-home',
  );
...
describe('scream export', () => {
  ...
  it('exits 1 when no session-id is provided and no previous session exists', async () => {
    ...
    expect(stderr.join('').toLowerCase()).toContain('未找到可导出的历史会话');
  });
  ...
  it('confirms before exporting the previous session when no id is supplied', async () => {
    ...
    expect(exitCodes).toEqual([]);
    expect(exportInputs).toEqual([]);
    expect(stdout.join('')).toContain('导出已取消。');
  ...
  it('describes the user-facing command without implementation details', () => {
    ...
    expect(command?.description()).toBe('导出会话为 ZIP 压缩包。');
  ...
  it('parses --no-include-global-log as an option when no session id is given', async () => {
    ...
    await program.parseAsync(['node', 'scream', 'export', '--no-include-global-log', '-y']);
