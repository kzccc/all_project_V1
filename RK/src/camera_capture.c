...
static void yuyv_to_rgb(const uint8_t *src, uint8_t *dst, int width, int height) {
    ...
    for (int i = 0; i < width * height * 2; i += 4) {
        ...
        for (int p = 0; p < 2; ++p) {
            ...
            dst[out++] = (uint8_t)(r < 0 ? 0 : r > 255 ? 255 : r);
            dst[out++] = (uint8_t)(g < 0 ? 0 : g > 255 ? 255 : g);
            dst[out++] = (uint8_t)(b < 0 ? 0 : b > 255 ? 255 : b);
        }
    }
}

static void nv12_planes_to_rgb(const uint8_t *yplane, const uint8_t *uvplane, uint8_t *dst, int width, int height) {
    int out = 0;
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            int yv = yplane[y * width + x];
...
static void nv12_to_rgb(const uint8_t *src, uint8_t *dst, int width, int height) {
    nv12_planes_to_rgb(src, src + (size_t)width * height, dst, width, height);
}
...
int main(int argc, char **argv) {
    ...
    printf("format: %dx%d %s type=%s planes=%u\n",
           width, height, fourcc,
        ...
        if (rgb) {
            if (pixfmt == V4L2_PIX_FMT_NV12) {
                if (buffers[frame.index].plane_count >= 2) {
                    ...
                    nv12_to_rgb((const uint8_t *)buffers[frame.index].start[0], rgb, width, height);
                }
            } else {
                yuyv_to_rgb((const uint8_t *)buffers[frame.index].start[0], rgb, width, height);
            }
