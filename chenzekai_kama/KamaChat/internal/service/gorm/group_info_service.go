...
func (g *groupInfoService) DismissGroup(ownerId, groupId string) (string, int) {
	...
	if res := dao.GormDB.Model(&model.GroupInfo{}).Where("uuid = ?", groupId).Updates(
		map[string]interface{}{
			"status":     group_status_enum.DISSOLVE,
			"deleted_at": deletedAt,
...
func (g *groupInfoService) DeleteGroups(uuidList []string) (string, int) {
	for _, uuid := range uuidList {
		...
		deletedAt.Time = time.Now()
		deletedAt.Valid = true
		if res := dao.GormDB.Model(&model.GroupInfo{}).Where("uuid = ?", uuid).Updates(
			map[string]interface{}{
				"status":     group_status_enum.DISSOLVE,
				"deleted_at": deletedAt,
				"updated_at": deletedAt.Time,
...
func (g *groupInfoService) CheckGroupAddMode(groupId string) (string, int8, int) {
	// 从数据库回源并写入缓存
	loadFromDB := func() (string, int8, int) {
		var group model.GroupInfo
		if res := dao.GormDB.First(&group, "uuid = ?", groupId); res.Error != nil {
			if errors.Is(res.Error, gorm.ErrRecordNotFound) {
				zlog.Error("群聊不存在")
		...
		if group.Status == group_status_enum.DISSOLVE {
			zlog.Info("群聊已解散")
			return "群聊已解散", -1, -2
		}
		rsp := respond.GetGroupInfoRespond{
		...
		if rspBytes, marshalErr := json.Marshal(rsp); marshalErr != nil {
			zlog.Error(marshalErr.Error())
		} else {
			if setErr := myredis.SetKeyEx("group_info_"+groupId, string(rspBytes), time.Minute*constants.REDIS_TIMEOUT); setErr != nil {
	...
	if err := json.Unmarshal([]byte(rspString), &rsp); err != nil {
		// 缓存数据损坏，删除缓存后回源数据库
		zlog.Error(err.Error())
		if delErr := myredis.DelKeyIfExists("group_info_" + groupId); delErr != nil {
			zlog.Error(delErr.Error())
		}
		return loadFromDB()
	}

	// 缓存命中，但仍要校验状态，避免旧数据放行
	if rsp.Status == group_status_enum.DISABLE {
		zlog.Info("群聊已被禁用")
		return "群聊已被禁用", -1, -2
	}
	if rsp.Status == group_status_enum.DISSOLVE {
		zlog.Info("群聊已解散")
		return "群聊已解散", -1, -2
	}
