...
class Fusionloss(nn.Module):
    ...
    def compute_loss(self, g_image):
        # result = torch.zeros_like(g_image)
        batch_size, channels, height, width = g_image.size()
        tensor_list = []
        # 通过第二维度（channels）遍历
        for i in range(channels):
            if i == 0:
                g_image_p = g_image[:,:(i+1),:,:]
                g_image_p = self.sobelconv(g_image_p)
                tensor_list.append(g_image_p)
                # g_image[:,:(i+1),:,:] = g_image_p
            else:
                g_image_p = g_image[:,(i):(i+1),:,:]
                g_image_p = self.sobelconv(g_image_p)
                tensor_list.append(g_image_p)
                # g_image[:,(i):(i+1),:,:] = g_image_p
        result = torch.cat(tensor_list, dim=1)
        return result
            # # 取出当前通道
            # channel_tensor = g_image[:, i, :, :].unsqueeze(1)  # 形状变为 (batch_size, 1, height, width)
            
            # # 应用 Sobel 边缘检测卷积
            # sobel_result = self.sobelconv(channel_tensor)
            
            # # 将结果存回结果张量中
            # result[:, i, :, :] = sobel_result.squeeze(1)
    ...
    def forward(self,image_vis,generate_img):
        ...
        generate_img_grad = self.compute_loss(generate_img)
        # generate_img_grad=self.sobelconv(generate_img)
