...
<script>
// 组件脚本负责管理聊天状态、消息收发、会话切换以及音视频信令处理。
import { computed, reactive, toRefs, onMounted, ref, nextTick, watch } from "vue";
...
export default {
  ...
  setup() {
    ...
    // data 集中管理当前聊天页的所有状态，包括会话信息、消息列表、群管理弹窗和音视频状态机。
    const data = reactive({
      chatMessage: "",
      chatName: "",
    ...
    const sortMessageList = () => {
      data.messageList.sort((left, right) => left.session_seq - right.session_seq);
    };

    const buildMessageId = () => {
      if (window.crypto?.randomUUID) {
        return `M${window.crypto.randomUUID().replace(/-/g, "")}`;
      }
      return `M${Date.now().toString(36)}${Math.random().toString(36).slice(2, 10)}`;
    };

    const getMessageDedupKey = (message) => {
      if (message == null) {
        return "";
      }
      ...
      if (message.session_seq != null && message.session_seq !== "") {
        return `session_seq:${message.session_seq}`;
      }
      return "";
    };

    const resetSeenMessageIds = () => {
      seenMessageIds.clear();
      seenMessageOrder.length = 0;
    };

    const rememberMessageId = (message) => {
      const dedupKey = getMessageDedupKey(message);
      if (!dedupKey || seenMessageIds.has(dedupKey)) {
        return;
      }
      seenMessageIds.add(dedupKey);
      seenMessageOrder.push(dedupKey);
      while (seenMessageOrder.length > MAX_SEEN_MESSAGE_IDS) {
        const expiredDedupKey = seenMessageOrder.shift();
        if (expiredDedupKey) {
          seenMessageIds.delete(expiredDedupKey);
        }
      }
    };
    ...
    const isDuplicateMessage = (message) => {
      const dedupKey = getMessageDedupKey(message);
      return !!dedupKey && seenMessageIds.has(dedupKey);
    };

    const normalizeMessage = (message) => {
      const nextMessage = { ...message };
      if (nextMessage.send_avatar && !nextMessage.send_avatar.startsWith("http")) {
        nextMessage.send_avatar = store.state.backendUrl + nextMessage.send_avatar;
      }
      return nextMessage;
    };
    ...
    const setMessageList = (messages) => {
      ...
      for (const message of messages || []) {
        ...
        nextMessageList.push(normalizedMessage);
        rememberMessageId(normalizedMessage);
      }
      data.messageList = nextMessageList;
      sortMessageList();
    };
    ...
    const appendMessage = (message) => {
      ...
      data.messageList.push(normalizedMessage);
      rememberMessageId(normalizedMessage);
      sortMessageList();
      return true;
    };

    const handleSocketMessage = (jsonMessage) => {
    ...
    const bindSocketMessageHandler = () => {
      if (!store.state.socket) {
        return;
      }
      store.state.socket.onmessage = handleSocketMessage;
    };
    ...
    // 路由参数变化意味着用户切换了聊天对象，需要重拉联系人信息、会话 ID 与历史消息。
    onBeforeRouteUpdate(async (to, from, next) => {
      ...
      bindSocketMessageHandler();
      scrollToBottom();
      next();
    });
    // 首次进入聊天页时执行与路由切换相同的初始化流程，并接管 WebSocket 收包逻辑。
    onMounted(async () => {
      try {
        /*  */
    ...
    // getChatContactInfo 用于拉取当前会话对象的资料，并补齐头像访问前缀。
    ...
    // getSessionId 负责确保当前聊天对象对应的会话已存在，并拿到后续发消息要用的 session_id。
    ...
    // handleCreateGroup 预留给聊天页内快速建群使用。
    ...
    // 下面一组方法用于控制联系人详情、群详情和群管理相关弹窗的开关状态。
    ...
    // handleAddGroupList 会拉取当前群聊的待处理入群申请，并把弹窗切换到可展示状态。
    ...
    // getMessageList / getGroupMessageList 负责根据当前会话类型回拉历史消息。
    ...
    // handleUploadSuccess 在文件上传完成后把文件消息补发到当前会话。
    ...
    // 下面一组方法负责音视频弹窗展示、信令交换和 WebRTC 连接状态流转。
    ...
    // createOffer 会在本端成为呼叫发起方后生成 SDP offer 并发送给对端。
    const createOffer = () => {
      var offerOpts = {
    ...
    // startCall 负责申请本地媒体流、初始化 RTCPeerConnection，并按角色决定是否先发 offer。
    ...
    // handleOfferSdp / handleAnswerSdp / handleCandidate 分别处理远端 SDP 与 ICE candidate。
