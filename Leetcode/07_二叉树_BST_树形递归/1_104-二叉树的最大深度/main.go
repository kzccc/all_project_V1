...
func main() {
	...
	fmt.Println(maxDepthReference(buildTreeReference(tokens)))
}
...
func maxDepthReference(root *TreeNode) int {
	if root == nil {
		return 0
	}
	left := maxDepthReference(root.Left)
	right := maxDepthReference(root.Right)
	if left > right {
		return left + 1
	}
	return right + 1
