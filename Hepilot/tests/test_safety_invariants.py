...
from hepilot import FakeModelClient, MiniAgent, SessionStore, WorkspaceContext
from hepilot import cli as mini_cli
from hepilot.task_state import TaskState
...
def test_risky_tool_deny_behavior(tmp_path):
    ...
    assert "run_shell rejected; choose a different action before retry" in process_notes
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=True,
    ), patch("hepilot.cli.OllamaModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = mini_cli.build_agent(args)
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("hepilot.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "hepilot.runtime"

    with patch("hepilot.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
