...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".hepilot" / "sessions")
...
def test_agent_runs_tool_then_final(tmp_path):
    ...
    assistant_tool_calls = [
        item for item in agent.session["history"] if item["role"] == "assistant" and "<tool>" in item["content"]
    ]
...
def test_second_model_prompt_preserves_assistant_tool_call_context(tmp_path):
    (tmp_path / "hello.txt").write_text("alpha\nbeta\n", encoding="utf-8")
    agent = build_agent(
        tmp_path,
        [
...
def test_agent_converts_model_exception_into_controlled_model_error(tmp_path):
    class FailingModelClient:
        supports_prompt_cache = False
        last_completion_metadata = {}

        def complete(self, prompt, max_new_tokens, **kwargs):
            raise RuntimeError("OpenAI-compatible request failed with HTTP 500: upstream exploded")
    ...
    assert answer == "Stopped after model error: OpenAI-compatible request failed with HTTP 500: upstream exploded"
    ...
    model_failed = next(event for event in trace_events if event["event"] == "model_failed")
    assert model_failed["error_kind"] == "http_5xx"
    assert model_failed["prompt_metadata"]["request_chars"] == len("Do the task")
...
def test_invalid_risky_tool_does_not_prompt_for_approval(tmp_path):
    ...
    process_notes = [note["text"] for note in agent.memory.to_dict()["episodic_notes"] if note.get("kind") == "process"]
    assert "write_file rejected; choose a different action before retry" in process_notes


def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".hepilot").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".hepilot" not in result
...
def test_repeated_identical_tool_call_is_rejected(tmp_path):
    ...
    process_notes = [note["text"] for note in agent.memory.to_dict()["episodic_notes"] if note.get("kind") == "process"]
    assert "list_files rejected; choose a different action before retry" in process_notes


def test_unknown_tool_records_rejected_process_note(tmp_path):
    agent = build_agent(tmp_path, [])

    result = agent.run_tool("missing_tool", {})

    assert result == "error: unknown tool 'missing_tool'"
    process_notes = [note["text"] for note in agent.memory.to_dict()["episodic_notes"] if note.get("kind") == "process"]
    assert "missing_tool rejected; choose a different action before retry" in process_notes
...
def test_openai_compatible_client_retries_non_json_response_before_failing_runtime():
    call_count = {"value": 0}
    ...
    def fake_urlopen(request, timeout):
        ...
        return FakeResponse(json.dumps({"output_text": "<final>ok</final>"}))
    ...
    with patch("urllib.request.urlopen", fake_urlopen), patch("time.sleep", return_value=None):
        result = client.complete("hello", 42)
...
def test_anthropic_compatible_client_retries_empty_text_response_before_failing_runtime():
    call_count = {"value": 0}
    ...
    def fake_urlopen(request, timeout):
        ...
        call_count["value"] += 1
        if call_count["value"] == 1:
            return FakeResponse(json.dumps({"content": [{"type": "thinking", "thinking": "hidden"}]}))
        return FakeResponse(json.dumps({"content": [{"type": "text", "text": "<final>ok</final>"}]}))
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "hepilot.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("hepilot.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "hepilot.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("hepilot.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("hepilot.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    runs_root = tmp_path / ".hepilot" / "runs"
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .hepilot/.\n"
    ...
    index_path = tmp_path / ".hepilot" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".hepilot" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".hepilot" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .hepilot/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".hepilot" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".hepilot" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(mini_pkg.__file__).as_posix().endswith("/hepilot/__init__.py")
