#!/usr/bin/env python3
"""批量执行 provider、resume、memory、context、安全实验的总入口。"""
...
from hepilot.metrics import (  # noqa: E402
...
def build_arg_parser():
    """声明大规模实验所需的输入、输出与运行模式参数。"""
    parser = argparse.ArgumentParser(description="Run Hepilot large-scale experiments and write all experiment artifacts.")
    parser.add_argument("--benchmark-artifact", required=True, help="Path to benchmark artifact JSON.")
    parser.add_argument("--runs-root", required=True, help="Path to .hepilot/runs root.")
...
def main(argv=None):
    """串联 provider 实验、resume 聚合与最终报告生成。"""
    ...
    # 这些工件面向不同分析视角拆开写，避免下游脚本总是从一个大 JSON 里切片。
