"""固定 benchmark harness 与评测入口。

这个模块负责把 benchmark 任务定义、fixture 工作区复制、脚本化模型输出、
verifier 校验和最终 artifact 组织在一起，形成可复现的评测流水线。
"""
...
# 这些 scripted 输出把 benchmark 固定成 deterministic harness，
# 让评测代码自身先能独立回归，不被真实模型波动掩盖。
...
class BenchmarkEvaluator:
    ...
    def __init__(
        ...
        self.workspace_root = Path(workspace_root) if workspace_root is not None else Path(
            tempfile.mkdtemp(prefix="hepilot-benchmark-")
        )
    ...
    def run_task(self, task):
        """在独立 fixture 副本里运行单个 benchmark 任务。"""
        task = dict(task)
        ...
        session_store = SessionStore(fixture_copy_root / ".hepilot" / "sessions")
        run_store = RunStore(fixture_copy_root / ".hepilot" / "runs")
...
def run_fixed_benchmark(
    ...
    """运行固定 benchmark harness，并返回 artifact。"""
