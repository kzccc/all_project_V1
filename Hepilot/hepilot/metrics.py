"""benchmark、运行工件与实验指标的聚合层。

这里既包含离线 artifact 聚合，也包含 synthetic/real 两类实验入口，以及
最终 Markdown 报告渲染。文件较大，但可以按“聚合函数 -> synthetic 实验 ->
real 实验 -> recovery/context/memory artifact 输出”理解。
"""
...
from .runtime import Hepilot, SessionStore
...
DEFAULT_CORE_REPORT_PATH = Path("docs/metrics/hepilot-benchmark-core-report.md")
...
def aggregate_benchmark_artifact(path):
    """从 benchmark artifact JSON 提取常用统计与原始行数据。"""
...
def aggregate_run_artifacts(runs_root):
    """聚合 `.hepilot/runs/` 下的 report 与 trace 指标。"""
...
def measure_feature_ablation_metrics(agent, user_message):
    """比较同一个 agent 在不同 feature flag 组合下的 prompt 表现。"""
    variants = {
        "full": {},
...
def build_stress_agent_metrics():
    """构造一个高 history / 高 note 的小型压力场景并测量 prompt。"""
    with tempfile.TemporaryDirectory(prefix="hepilot-metrics-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        workspace = WorkspaceContext.build(workspace_root)
        store = SessionStore(workspace_root / ".hepilot" / "sessions")
        agent = Hepilot(
...
class _MemoryExperimentModelClient(FakeModelClient):
    """用于 memory 实验的脚本化模型，按是否命中记忆决定是否补读文件。"""
...
def _run_memory_variant(mode):
    with tempfile.TemporaryDirectory(prefix="hepilot-memory-experiment-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
# 大规模 memory 实验把任务拆成多类记忆依赖场景，
# 用来观察 memory 开关对 repeated read 的整体影响。
...
def _run_memory_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="hepilot-memory-large-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def run_context_stress_matrix(repetitions=5):
    """在不同 history / note / request 规模下测量上下文压缩表现。"""
    repetitions = int(repetitions)
    ...
    for history_label, history_count in history_levels:
        for note_label, note_count in note_levels:
            for request_label, request_text in request_levels:
                per_run = []
                for _ in range(repetitions):
                    with tempfile.TemporaryDirectory(prefix="hepilot-context-matrix-") as temp_dir:
                        workspace_root = Path(temp_dir)
                        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                        workspace = WorkspaceContext.build(workspace_root)
                        store = SessionStore(workspace_root / ".hepilot" / "sessions")
...
def run_security_experiment_suite(repetitions=3):
    ...
    for scenario_id, runner in SECURITY_SCENARIOS:
        for _ in range(repetitions):
            with tempfile.TemporaryDirectory(prefix="hepilot-security-exp-") as temp_dir:
                workspace_root = Path(temp_dir)
                (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def run_provider_experiments(benchmark_path, workspace_root, artifact_root, max_new_tokens=64):
    """在真实 provider 上跑 benchmark，并统一整理结果。"""
...
def run_real_memory_experiment(provider="gpt", repetitions=1):
    ...
    for task in MEMORY_EXPERIMENT_TASKS:
        ...
        for _ in range(repetitions):
            for variant in variants:
                with tempfile.TemporaryDirectory(prefix="hepilot-real-memory-") as temp_dir:
                    workspace_root = Path(temp_dir)
                    (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                    _write_memory_task_files(workspace_root, task)
...
def run_real_context_experiment(provider="gpt", repetitions=1):
    """在真实 provider 上执行 context stress 实验。"""
    repetitions = int(repetitions)
    provider = str(provider)
    history_levels = [("short", 4), ("medium", 12), ("long", 24)]
    note_levels = [("low", 2), ("high", 10)]
            ...
            for request_label, request_text in request_levels:
                ...
                for _ in range(repetitions):
                    for variant_name, updates in (("full", {}), ("no_context_reduction", {"context_reduction": False})):
                        with tempfile.TemporaryDirectory(prefix="hepilot-real-context-") as temp_dir:
                            workspace_root = Path(temp_dir)
                            (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                            agent = _build_real_agent(workspace_root, provider)
                            for index in range(note_count):
...
def _run_real_repeated_call_scenario(provider):
    with tempfile.TemporaryDirectory(prefix="hepilot-real-security-repeat-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        agent = _build_real_agent(workspace_root, provider)
...
def run_real_security_experiment_suite(provider="gpt", repetitions=1):
    ...
    for _ in range(repetitions):
        ...
        for scenario in REAL_SECURITY_SCENARIOS:
            with tempfile.TemporaryDirectory(prefix="hepilot-real-security-") as temp_dir:
                workspace_root = Path(temp_dir)
...
def collect_resume_metrics(
    ...
    """聚合 benchmark、runs 和实验结果，生成一份 resume 指标总表。"""
...
def render_large_scale_experiment_report(metrics):
    ...
    lines = [
        "# Hepilot Large-Scale Experiment Report",
        "",
        "## Executive Summary",
        (
...
def _run_recovery_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="hepilot-recovery-ablation-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def run_context_ablation_v2(artifact_path=DEFAULT_CONTEXT_ABLATION_V2_PATH, repetitions=5):
    """生成并写出 context ablation v2 artifact。"""
...
def run_memory_ablation_v2(artifact_path=DEFAULT_MEMORY_ABLATION_V2_PATH, repetitions=5):
    """生成并写出 memory ablation v2 artifact。"""
...
def run_recovery_ablation_v2(artifact_path=DEFAULT_RECOVERY_ABLATION_V2_PATH, repetitions=3):
    """生成并写出 recovery ablation v2 artifact。"""
    repetitions = int(repetitions)
...
def write_benchmark_core_report(
    ...
    """汇总四类核心 benchmark artifact，生成项目级核心报告。"""
